...
3. **Plant control depth:** maintenance-to-order linkage, WIP valuation projection, training/document control, and governed recall execution remain. Item/BOM/routing effective dates, material return/substitution, recurrence-linked quality-problem CAPA with due-dated review and read-only portfolio trend control, and a multi-level batch recall trace/export are implemented; none blocks inventory, contacts customers, issues certificates, or decides a recall.
