...
- **Plant:** reviewed BOM/routing, material and capacity checks, exact Shop issue handoff, routed WIP/time, output/scrap, inspection/hold, attributed rework/corrective action, reinspection/release, recurrence-linked CAPA with due-dated effectiveness review and portfolio trend control, genealogy, effectiveness/OEE, standard-versus-actual MMK cost, and versioned work-centre calibration evidence are implemented. New v3 controlled plans block release, routed work, direct reinspection after failure, and rework without current calibration; legacy v1/v2 plans remain readable.
...
3. **Plant control depth:** maintenance duration/window scheduling, WIP valuation projection, training/document control, and governed recall execution remain. Item/BOM/routing effective dates, material return/substitution, recurrence-linked quality-problem CAPA with due-dated review and read-only portfolio trend control, maintenance-to-controlled-order evidence, deterministic current-load review, and a multi-level batch recall trace/export are implemented; none blocks inventory, reschedules production, contacts customers, issues certificates, commands equipment, or decides a recall.
...
Checkpoint `79112bcc` adds the backward-compatible `supermega.production.quality-capa.v2` close contract and `supermega.production.quality-capa-effectiveness.v1` review contract inside the same Problems workflow. A new CAPA cannot close without a due date for one separate human review; the review cannot happen early or twice. Browser and managed runtimes derive classified recurrence since close from immutable issue history. A recurrent CAPA cannot be marked effective, while either recurrence or an ineffective finding requires escalation. Legacy v1 records remain readable. The review records evidence and accountability only: it does not reopen a problem, release a batch, block stock, contact a customer, issue a certificate, or control equipment.

Checkpoint `4419e427` adds read-only `supermega.production.quality-capa-trend.v1` parity. Exact recurrence keys group CAPAs into one ranked control: recurrence or ineffective review escalates, a due review needs action, legacy or unclassified history is an evidence gap, and future review stays monitored. The projection is deterministic and cannot reopen problems, release batches, block stock, contact customers, issue certificates, or control equipment.

Checkpoint `4aabf9c5` adds backward-compatible `supermega.production.maintenance-finding-source.v2` parity. A restrictive, strategy-bound maintenance completion snapshots its exact equipment work centre and every unreleased controlled order routed through it, including the order revision/head, approved plan digest, and matching operations. Opening the linked problem re-derives the snapshot so omissions and forged routing fail closed; later order commands do not rewrite retained evidence. Legacy v1 findings remain readable. This is review evidence only: it does not hold or reschedule orders, change inventory, or command equipment.

Checkpoint `b8867924` adds read-only `supermega.production.maintenance-capacity-review.v1` parity. Every due maintenance asset is joined through its exact work centre to unreleased controlled orders and matching unfinished operations, retaining job owner/priority/due time, plan and order identity, remaining quantity, and reviewed minutes. Deterministic status distinguishes overdue, due-soon, and planned work with or without load. The compact review stays inside Maintenance and grants no maintenance scheduling, order rescheduling, machine-status, equipment, inventory, or job authority. Planned duration/window optimization remains unimplemented.
