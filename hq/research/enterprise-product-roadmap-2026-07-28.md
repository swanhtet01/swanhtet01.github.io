...
3. **Plant control depth:** capacity-aware maintenance planning, WIP valuation projection, training/document control, and governed recall execution remain. Item/BOM/routing effective dates, material return/substitution, recurrence-linked quality-problem CAPA with due-dated review and read-only portfolio trend control, maintenance-to-controlled-order evidence, and a multi-level batch recall trace/export are implemented; none blocks inventory, reschedules production, contacts customers, issues certificates, commands equipment, or decides a recall.
