...
This follows the enterprise pattern in current SAP guidance: tax codes are configuration records determined from document attributes, and tax codes also determine the applicable tax account. SuperMega therefore must not invent a Myanmar rate, hidden default, or G/L account. The next finance slice requires explicit account mapping, correction documents, jurisdiction/effective-date handling, and an owner-approved duplicate-safe adapter before any posting claim.
