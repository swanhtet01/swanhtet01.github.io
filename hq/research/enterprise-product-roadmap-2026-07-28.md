...
3. **Plant control depth:** broader CAPA recurrence control, maintenance-to-order linkage, WIP valuation projection, training/document control, and governed recall execution remain. Item/BOM/routing effective dates, material return/substitution, and a read-only multi-level batch recall trace/export are implemented; the trace deliberately does not block inventory, contact customers, or decide a recall.
