...
Previous local checkpoint: `be8ac2e4cf8679bdebcdabd9188539e49bffd9f4`
...
| ENG-010 | Plant Codex | queued | Let operators record any truthful distinct equipment observation instead of a forced cycle. | Existing Problems tab, machine-state event, mirrored runtime, and focused tests only | All six valid transitions pass; same-state, stale, unknown, and conflicting transitions fail closed; wording remains observation-only and mobile-safe. |
| ENG-011 | Website Codex | queued | Stage Website edits behind one Save or Discard boundary. | Existing Website editor, local draft buffer, focused styles and tests only | Typing causes zero authoritative revisions; one Save creates one revision; Discard and stale-base recovery fail safely; publish/evidence actions remain blocked while edits are unsaved. |
