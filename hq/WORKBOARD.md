...
Previous local checkpoint: `4aa203c`
...
| ENG-016 | Plant Codex | queued | Make every Plant problem actionable with severity, owner, due time, containment, and evidence-backed closure. | Existing Problems tab, issue event contract, mirrored runtime, and focused tests only | Historical issues remain readable; every new open issue has severity, owner, due time, and next containment action; every close retains a named human and evidence; overdue and high-severity work is obvious without a new tab, notification system, telemetry, equipment command, hosted write, or deployment. |
...
1. Execute ENG-016 as the next bounded Plant slice inside Problems, preserving the existing accountable action gate and historical records.
