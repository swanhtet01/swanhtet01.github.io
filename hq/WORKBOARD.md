...
Current accepted product checkpoint: `3d4d5bb`
...
| ENG-020 | Ecommerce Codex | done-local | Keep mobile storefront setup and customer preview understandable without one long stacked workspace. | Existing Ecommerce component, focused styles, and app-build verifier only | Checkpoint `92cb497` adds one 46 px sticky Setup/Preview switch below 840 px, renders only the selected mobile workspace, preserves both desktop panels, and scrolls the selected panel below fixed navigation. A 375 px two-way journey passes with no undersized controls, horizontal overflow, overlays, warnings, or console errors; lint, build, the full local app release contracts, and all 122 Python tests pass. No product data, request contract, Shop authority, route, backend, push, hosted write, or deployment changed. |
...
| OPS-002 | Release / Codex integrator | done-local | Reconcile canonical domains, Vercel projects, and Git commits before any release action. | Read-only GitHub, Vercel, DNS, and local Git evidence only | At the last live audit, `supermega.dev` and `app.supermega.dev` served `main` checkpoint `6885c320`; local `3d4d5bb` is 115 commits ahead of the locally tracked `origin/main` and 69 ahead of the tracked integration branch. The Vercel project named `supermega-platform` built `swanhtet01.github.io` rather than this checkout. Demo, console, and one retired legacy alias remained separate deployments. No push, merge, deployment, alias, domain, or project setting changed. |
| OPS-003 | Data / Codex integrator | done-local | Determine whether an isolated managed persistence rehearsal target already exists. | Read-only Supabase project, table, and Security Advisor inventory only | The one connected healthy PostgreSQL 17 project contains the older public-schema system and no `app_private` tables; the reviewed production ref is still unset and no development branch was verified. Twenty-seven informational advisor notices report RLS-enabled public tables with no policies. The existing project is not accepted as the isolated rehearsal target, and no SQL, branch, key, project, or configuration changed. |
...
1. Keep `3d4d5bb` as the local release-candidate checkpoint and review its 115-commit delta before any owner-approved push, merge, deployment, or domain change.
