...
| ENG-019 | Plant Codex | queued | Record one bounded downtime interval without pretending machine telemetry or control. | Existing Problems tab, Production event contract, mirrored runtime, and focused tests only | An accountable human starts downtime for one recorded machine with reason and evidence, and a later accountable human ends it with new reason and evidence. At most one interval is open per machine; timestamps cannot run backward; duration is derived; exact replay is safe; current and recent downtime are obvious without changing machine state, output, holds, issues, or stock. Historical workspaces default to no downtime. No OEE dashboard, automatic sensor input, equipment command, alerting system, predictive maintenance, new tab, hosted write, or deployment is added. |
