...
| ENG-015 | Plant Codex | queued | Record good and scrap quantities separately without letting a job exceed its approved target. | Existing Jobs tab, output event contract, mirrored runtime, and focused tests only | Historical jobs remain readable as zero recorded scrap; every scrap entry is shift-attributed and human-evidenced; good plus scrap cannot exceed target; exact retries are idempotent; no new tab, MRP/OEE scope, telemetry, equipment command, hosted write, or deployment. |
...
1. Execute ENG-015 as the next bounded Plant slice inside Jobs, preserving the existing accountable action gate and historical records.
