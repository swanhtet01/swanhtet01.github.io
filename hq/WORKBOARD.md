...
Previous local checkpoint: `65992f289cc7c9af5bbcfae0bf7af42a9d705c55`
...
| ENG-014 | Shop Codex | queued | Let one Shop order contain multiple items while freezing each MMK line price at confirmation. | Existing New Order workflow, order contract, mirrored runtime, and focused tests only | Historical single-item orders remain lossless; every line is validated atomically; reservation and cancellation affect every line exactly once; no new tab, pricing service, payment provider, or hosted write. |
...
1. Execute ENG-014 as the next bounded product slice, preserving historical Shop orders and the existing accountable action gate.
