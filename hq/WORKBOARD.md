...
| ENG-018 | Plant Codex | queued | Generate one evidence-backed shift handoff from current Plant records. | Existing Jobs/Problems tabs, deterministic read-only derivation, focused UI, and verifier only | A named shift reference produces one compact handoff containing every unfinished job, active quality hold, open critical/high problem, and latest recorded machine observation or explicit no-observation marker. Every statement links to existing record evidence, no unsupported causal claim is added, and generating or copying the handoff changes no Plant state. No model call, event, persistence, message send, export suite, new tab, hosted write, or deployment is added. |
...
1. Execute ENG-018 as the next bounded Plant slice inside the existing Plant tabs, deriving the handoff from validated records without adding a second source of truth.
