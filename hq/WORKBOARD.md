...
| ENG-017 | Plant Codex | queued | Add an explicit quality hold and evidence-backed release for an existing job or batch. | Existing Jobs/Problems tabs, production event contract, mirrored runtime, and focused tests only | Historical jobs remain unheld; hold and release are distinct immutable events; the current hold state, owner, reason, and evidence are obvious; release requires a named human decision and nonblank evidence; neither action changes output, scrap, equipment, or stock and no new tab, lot-traceability suite, hosted write, or deployment is added. |
...
1. Execute ENG-017 as the next bounded Plant slice inside the existing Plant tabs, preserving the accountable action gate and historical records.
