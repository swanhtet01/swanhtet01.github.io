...
1. **The FOUC window is UNVERIFIED, and my first attempt to verify it was
   wrong.** I probed for "mounted but unstyled" by watching for a specific
   background colour and concluded the stylesheet never applied at all
   (`cssAppliedMs: null`, 92 unstyled samples). Both conclusions were false —
   the sentinel colour was wrong, and the direct comparison above shows the CSS
   loads and applies fine. A broken *check* looks exactly like a broken
   *feature*; the end-state comparison is the assertion to trust, and it is the
   one that should be written first next time. What remains genuinely unknown
   is whether there is a window where React has mounted (retiring the boot
   shell) before the stylesheet applies. It is unlikely — the CSS is far
   smaller than the entry set and no longer queues behind it — but "unlikely"
   is not "measured", and this is precisely the kind of gap that produced the
   two rejected optimizations above.
2. `css-async.js` must be generated by `tools/write_app_release_metadata.mjs`
   like the other three shell scripts, pinned by `verify_app_build.mjs`
   alongside them, and added to the offline precache list (35 files today).
3. `verify_app_build.mjs` pins shell contents; whether it asserts the stylesheet
   link shape has not been checked.
