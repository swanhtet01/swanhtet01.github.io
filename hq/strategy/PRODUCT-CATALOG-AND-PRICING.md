...
2. The pre-registered order-intake evaluation run green, with a named pilot operator and the correction-effort bar met. State as of run 5 (2026-08-17): a deterministic post-model guard closed BOTH zero-tolerance safety gates for the first time (zero fabricated critical facts, zero unsafe ready-for-review) — real progress on the harder problem. The overall gate still fails on 2 pre-existing provider schema-errors and quote-literal wording variance (model non-determinism). Capability stays dark; see `hq/research/order-intake-agent-evaluation-2026-08.md` run log.
