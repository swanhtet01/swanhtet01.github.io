...
What premium requires in practice: a relationship with SuperMega-run infrastructure — the AI intake path is served from the managed tenant runtime, so premium is not a switch a free user can flip alone. And the plain fact, updated 2026-08-17: **premium still has no purchase path a founder can run.** `premiumUnlocked` now has a real backing mechanism (`billing_rail.py`'s `BillingLedger`, migration v12 — invoice/event/entitlement tables, RLS deny-by-default, founder-evidenced transitions) and even a read path into `TrialReadiness` (fail-closed dark pending migration v13's narrow entitlement-read grant, drafted and locally verified). What's still missing is the operational entrypoint: no HTTP route, no founder CLI wraps `BillingLedger`'s issue/confirm/grant methods, so there is no way to actually run an invoice through it today. `BILLING-RAIL-DESIGN.md` section 6 has the itemized remaining estimate (roughly one focused day). This was COORDINATION-BOARD 2026-08-06's Gate 9 (CODE-MISSING) — the code is no longer missing; the wiring is.
...
3. An operational entrypoint for the billing rail — the code and data model exist (migration v12, `billing_rail.py`) but nothing calls it outside its own tests yet. Roughly a day of wiring per `BILLING-RAIL-DESIGN.md` section 6.
