...
**The honest one-line state:** all four local products are proven and free forever by build-checked invariant; hosted self-serve tenancy is proven six-for-six — on isolated, since-deleted infrastructure, not production; go-live now waits on one founder activation decision (a 4-step, mostly reversible runbook); and revenue is zero — the billing rail's CODE now exists (migration v12 + `supermega_runtime/billing_rail.py`, landed 2026-08-17, tested) but is not wired to anything a founder or customer can actually call yet (no HTTP route, no CLI entrypoint outside its own test), so **"can take money" is still false in practice** even though "no billing rail exists" (COORDINATION-BOARD 2026-08-06, Gate 9) is no longer accurate either. See section 4.
...
3. Migration v13 (the entitlement-read grant) proven on a disposable branch and applied — the CLI to actually run a billing transition is built and tested as of 2026-08-17; what's left is proving the read path live and the pricing decision.
...
2. **Billing rail wiring** — same gap as premium item 3: the data model and RLS are built and reviewed (migration v12; v13's narrow entitlement-read is drafted, locally verified, unapplied); no founder-runnable entrypoint exists yet.
