...
- Product outcome gates pass. OPS-242 returns confirmed business/location review directly to Sell, focuses search, and shows the frozen physical location without creating a sale or moving stock. Next: `shop-stock-location-code-alignment-client-acceptance`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-242 at `56d87229` waits for human scope confirmation to finish, opens the existing Sell task, focuses item search, and renders the reviewed `LOC-*` in the counter. CLAUDE-008 stays manual at `969fae03`.
...
1. Complete `shop-stock-location-code-alignment-client-acceptance`: when the founder changes the canonical stock location, replace only a blank or previously derived location code while preserving an explicitly edited code.
