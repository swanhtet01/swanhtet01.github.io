...
- 29 recoveries/audits and all 32 Shop/16 Plant final actions pass. Plant now detects repeated IDs across every nonempty CSV row. Next: `plant-job-import-priority-repair`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-222 prevents blocked-first Job IDs from being accepted later in the same CSV without creating records. CLAUDE-007 stays manual at `2744b28f`.
...
1. Complete `plant-job-import-priority-repair`: reject unknown nonempty priority values instead of silently changing them to normal.
