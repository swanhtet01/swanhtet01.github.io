...
- Ally binds one specialist, no external/Vercel/scheduler actions, and 4,096/768/0s. Shared work is one digest-bound project with four-default/six-max serial roles; changes require an idle atomic handoff. `supermega.ally-working-set-trim.v1` released 2,949.3 MB without Codex process stops. The Vision retry failed with no report. Audit retains one frontend, backend, idle worker, zero models/subagents, and one-run admission. Four CEO outcomes are accepted; Finance/Risk remains quarantined.
