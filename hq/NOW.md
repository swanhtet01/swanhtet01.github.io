...
- 24 recoveries, exact supersession, request-first Shop review, explicit order creation, and exact final actions for all 16 Plant workflows pass. Next: `shop-accountable-action-language-completion`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-201 exhaustively binds all 16 Plant action kinds to exact final labels while preserving evidence, recovery, append-only authority, and the generic Shop fallback. CLAUDE-006 remains manual and read-only.
...
1. Complete `shop-accountable-action-language-completion`: inventory every Shop action kind and replace generic final buttons with exact imperatives while preserving evidence, recovery, and sole Shop order/stock/payment authority.
