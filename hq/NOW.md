...
- 30 recoveries/audits and all 32 Shop/16 Plant final actions pass. Plant now accepts explicit priority aliases and blocks unknown nonempty values. Next: `plant-job-import-template-download`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-223 prevents unknown Plant CSV priorities from silently becoming normal and removes a redundant count checklist without creating records. CLAUDE-007 stays manual at `2744b28f`.
...
1. Complete `plant-job-import-template-download`: let operators download the exact Plant CSV headers and one safe example from the import surface.
