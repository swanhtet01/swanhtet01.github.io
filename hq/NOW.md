...
- 24 recoveries, exact supersession, request-first Shop review, explicit order creation, and exact Plant output actions pass. Next: `plant-material-confirm-action-clarity`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-199 gives reviewed Plant results exact `Record output` and `Record scrap` actions while preserving the evidence gate, append-only write path, and recovery. CLAUDE-006 remains manual and read-only.
...
1. Complete `plant-material-confirm-action-clarity`: replace the generic Plant material-use `Confirm change` action with exact record-material language while preserving append-only evidence and recovery.
