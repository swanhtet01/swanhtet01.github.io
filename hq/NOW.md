...
- 31 recoveries/audits and all 32 Shop/16 Plant final actions pass. Plant now downloads and round-trips an exact reusable CSV template. Next: `plant-job-import-next-ready-focus`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-224 gives Plant a compact template-first CSV path with fresh valid example data and no record creation. CLAUDE-007 stays manual at `2744b28f`.
...
1. Complete `plant-job-import-next-ready-focus`: after one reviewed imported job is confirmed, prepare the next ready row without bypassing per-job approval.
