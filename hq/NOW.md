...
- Checkpoint `8182491` is the accepted local product and database-authority baseline.
...
- Current local `8182491` is a fast-forward 140 commits beyond remote PR #258 head `338b6fd` and 186 beyond `main` `6885c320`; existing green checks cover only the remote head. The external mapping audit is anchored at `49b4e0e`; its later local checkpoints are not live.
...
- Live GitHub `main` is 186 commits behind the accepted local product checkpoint, while draft PR #258 is 140 commits behind it. The live app cannot contain the validated local work until owner-approved integration and a later coordinated release.
