...
- 24 recoveries, exact supersession, request-first Shop review, explicit order creation, and exact Plant output/material-use actions pass. Next: `plant-accountable-action-language-completion`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-200 moves exact final actions into one typed map and adds `Record material use` without changing evidence, recovery, or append-only authority. CLAUDE-006 remains manual and read-only.
...
1. Complete `plant-accountable-action-language-completion`: inventory every remaining Plant action kind and replace generic final buttons with exact imperatives while preserving evidence, recovery, and append-only authority.
