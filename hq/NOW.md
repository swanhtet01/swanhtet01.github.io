...
- 25 recoveries and exact final actions for all 32 Shop plus 16 Plant action kinds pass. Plant Today now focuses both output and the highest-priority real blocker on desktop and mobile. Next: `plant-today-material-cold-load`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-210 carries Plant blocker intent across tabs, preserves priority order, and focuses the real review action. CLAUDE-007 remains a manual read-only review of exact checkpoint `2744b28f`.
...
1. Complete `plant-today-material-cold-load`: retain material-entry intent until the first active job is loaded and focusable.
