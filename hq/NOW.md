...
- 28 recoveries/audits and all 32 Shop/16 Plant final actions pass. Plant CSV limits now match the governed Add-job fields. Next: `plant-job-import-all-row-repeat-detection`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-221 blocks overlong Plant CSV identity, line, product, and owner values without creating records. CLAUDE-007 stays manual at `2744b28f`.
...
1. Complete `plant-job-import-all-row-repeat-detection`: reject a repeated Job ID even when its first row is blocked for a different rule.
