...
- Public states the offer: four browser-local products stay free; managed adds approved cross-product context, company history and roles, reviewed actions, recovery, and support only after identity, tenant, recovery, and write controls pass (`25cac2f5`).
...
- Checks: 106 Ecommerce, 342 Commerce, 327 Production, 128 Shop next-action, 58 AR customer receivables, 91 Plant quality (25 quality hold + 66 CAPA), 109 Website, 95 security, 241 onboarding, 11 backup, 85 managed import, and 56 PostgreSQL 17 checks across eleven migrations. Shop next-action raised from 10 → 128 (all 9 decision paths, priority ordering, plural/singular, validation). AR receivables and Shop next-action both wired into app:verify. Plant quality CAPA test covers buildProductionQualityCorrectiveAction, resolve with/without CAPA, idempotency, recurrence detection, Unicode normalization, and tamper detection.
