...
- Plant maintenance binds active strategy, projects due work, requires structured reviewed completion results, and lets a named human turn a restricted or not-recommended result into exactly one evidence-linked problem review. It performs no automatic problem opening, dispatch, control, telemetry, status, or parts action.
...
- The candidate passes 380 Python, 274 Production, 318 Kernel, 69 connector, and 15 crew checks. Earlier Headless Edge evidence covers Plant maintenance at desktop and 390 px; the latest browser webview did not attach, so ENG-125 makes no new rendered claim.
