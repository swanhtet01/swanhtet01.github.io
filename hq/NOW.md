...
- 25 recoveries and exact final actions for all 32 Shop plus 16 Plant action kinds pass. Plant Today now focuses output, the highest-priority blocker, and material entry on desktop and mobile. Next: `plant-today-plan-focus`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-211 keeps Plant material intent through cold-load and focuses the real material reference. Historical local receipts are digest-bound outside the hot portfolio without weakening duplicate-work rejection. CLAUDE-007 remains a manual read-only review of exact checkpoint `2744b28f`.
...
1. Complete `plant-today-plan-focus`: carry the no-active-job planning intent across tabs and focus the existing Add job form.
