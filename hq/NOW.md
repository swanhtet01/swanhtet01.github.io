...
- Shop runs at `/shop/` with guarded Orders and Stock: multi-item orders, recovery, owner/promise fulfilment, payments/refunds, counts, purchasing, close, returns, and guarded catalog edits. Confirmation names the visible owner; promises drive queue order and live risk labels. Legacy records stay readable without invented owners or deadlines. Every edit has proof.
...
- Checkpoint `e3f6e18` is the accepted local product and database-authority baseline.
...
- HQ remains its own route chunk; the largest JavaScript chunk is 495,204 bytes, leaving 4,796 bytes under the hard gate.
- Focused coverage: 192 Shop, 34 order-recovery, 250 Plant, 94 Website, 11 managed-Website, 27 storefront-draft, 14 storefront, 15 request, 17 managed-storefront, and 16 handoff checks.
- Rendered Shop QA covers core flows plus promises, owners, and deadline risk. At 390 px, owner and Late labels fit with 44 px controls and no overflow; 1280 px is clean and error-free.
...
- Local `e3f6e18` is +170 over cached `origin/main` `6885c320` and +124 over cached release-candidate tracking `338b6fd`. Live GitHub/Vercel state was not refreshed; no push, merge, or deployment occurred.
...
- Cached GitHub `main` is 170 commits behind this product checkpoint. The live app cannot contain the validated local work until an owner-approved integration and coordinated release.
...
2. Review the 166-commit release-candidate delta, then obtain explicit owner approval before any push, merge, deployment, or canonical-domain correction.
