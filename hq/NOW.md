...
- OPS-258 preserves the exact Ecommerce request and Shop order through matched Daily close and both review-only CSVs. Next: `ecommerce-local-multiline-order-lifecycle-acceptance`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-258 checkpoint `0a53861a` upgrades close and accounting exports to v6 so exact Ecommerce request/order IDs survive deterministic digests and CSV download; customer text remains excluded and posting authority remains none. CLAUDE-009 stays manual at `0fc5cf7d`; its one safe-mode CLI attempt stopped before inference with `Not logged in`, zero tokens, and $0.00 cost, without touching the founder's session.
...
1. Complete `ecommerce-local-multiline-order-lifecycle-acceptance`: prove one two-SKU cart through current Shop review, per-SKU stock, fulfilment, matched close, source-bound exports, reload, and mobile UI.
