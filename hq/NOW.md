...
- 26 recoveries/audits and all 32 Shop/16 Plant final actions pass. Plant accepts a corrected CSV under the same filename; every review remains local. Next: `plant-job-import-latest-file-wins`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-219 proves same-name Plant CSV retry and adds a regression contract without changing app bytes or creating records. CLAUDE-007 stays manual at `2744b28f`.
...
1. Complete `plant-job-import-latest-file-wins`: prevent an older slow local file read from replacing a newer Plant CSV review.
