...
Agent Solutions stays gated R&D until these four pass onboarding, import, recovery, security, and pilot evidence.
...
1. **Shop + Plant** — versioned client templates and dry-run catalog/job imports; row errors, no silent writes.
2. **Website + Ecommerce** — industry starters, guided mapping, Preview, Review, and deterministic handoff.
...
- Current local `5d7b217` is a fast-forward 164 commits beyond remote PR #258 head `338b6fd` and 210 beyond `main` `6885c320`; existing green checks cover only the remote head. The external mapping audit is anchored at `49b4e0e`; its later local checkpoints are not live.
...
- Delivery stays on Shop, Plant, Website, and Ecommerce; Agent Solutions remains gated R&D until these four pass client onboarding and pilot gates.
...
1. Define one versioned client-template/import contract across the four active products, then implement a Shop catalog CSV dry run with mapping, row errors, duplicates, and zero writes before confirmation.
