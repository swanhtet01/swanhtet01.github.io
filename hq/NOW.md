...
- 24 recoveries, exact supersession, request-first Shop review, and explicit order creation pass. Next: `plant-output-confirm-action-clarity`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-198 gives every order-creation gate an exact `Create order` action while leaving non-order confirmation and write authority unchanged. CLAUDE-006 remains manual and read-only.
...
1. Complete `plant-output-confirm-action-clarity`: replace the generic Plant output `Confirm change` action with exact record-output language while preserving append-only evidence and recovery.
