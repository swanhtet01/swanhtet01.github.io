...
- Product outcome gates pass. OPS-243 keeps derived Shop location codes aligned with the selected `LOC-*` while preserving deliberate custom codes and making no accounting write. Next: `shop-unscoped-close-guidance-client-acceptance`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-243 at `dbbc6cf9` updates only blank or previously derived Shop location codes when the founder changes the canonical stock location; explicit custom codes remain untouched. CLAUDE-008 stays manual at `969fae03`.
...
1. Complete `shop-unscoped-close-guidance-client-acceptance`: before a reviewed business/location exists, replace the duplicated `Legacy unscoped` close label with one clear setup-required state while keeping historical unscoped records readable.
