...
- 25 recoveries, exact supersession, request-first Shop review, and exact final actions for all 32 Shop plus 16 Plant action kinds pass. Shop finance attention now has one visible secondary action. Next: `shop-finance-control-simplification`; pilots stay owner-gated.
...
- Codex is sole integrator. OPS-204 replaces Today’s secondary action with `Open finance` only when payment review or daily close needs attention; otherwise `New sale` remains. CLAUDE-006 remains manual and read-only.
...
1. Complete `shop-finance-control-simplification`: separate daily close from promotions, credit, and tax inside the existing Orders screen without adding a page or changing authority.
