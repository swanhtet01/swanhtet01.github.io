...
- Plant runs at `/plant/` with Jobs and Problems; the old operations URL is compatibility-only. It covers scheduled jobs, short closes, output, materials, quality, equipment observations, downtime, owned maintenance, and handoff. Priority and due time set one evidence-bound run order without controlling equipment or changing purchasing, costing, or accounting.
...
- Checkpoint `42fa9cf` is the accepted local product and database-authority baseline.
- App lint/build, all local release/security/database/HQ contracts, and all 153 Python tests pass. Security coverage is 58 checks.
- The Showroom dependency audit reports zero known vulnerabilities; React Router is on patched v8 and Node is `>=22.22.0`.
- HQ remains its own route chunk; the largest JavaScript chunk is 487,140 bytes, leaving 12,860 bytes under the hard gate.
- Focused coverage includes 178 Shop, 32 order-recovery, 230 Plant, 94 Website, 11 managed-Website, 27 storefront-draft, 14 storefront, 15 request, 17 managed-storefront, and 16 Ecommerce handoff checks.
...
- Rendered Plant QA covers material use, short close, maintenance, and `JOB-QA-URGENT` creation. At 390 px its exact schedule review, first-place run order, output selection, and reload persistence pass; wide layout and browser logs are clean.
...
- Local `42fa9cf` is +160 over cached `origin/main` `6885c320` and +114 over cached release-candidate tracking `338b6fd`. Live GitHub/Vercel state was not refreshed; no push, merge, or deployment occurred.
...
- Cached GitHub `main` is 160 commits behind this product checkpoint. The live app cannot contain the validated local work until an owner-approved integration and coordinated release.
...
2. Review the 160-commit release-candidate delta, then obtain explicit owner approval before any push, merge, deployment, or canonical-domain correction.
