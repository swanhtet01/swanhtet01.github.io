...
- Ecommerce reads Shop data. Setup, Save, receipt, and handoff focus the next required step. Review labels source, customer, SKU/price, payment, full promise, fulfilment, order ID, and stock delta. Stale or legacy requests fail closed; no handoff reserves stock or creates an order before accountable confirmation.
...
- Checkpoint `d941cad` is the accepted local product and database-authority baseline.
...
- React Router is isolated in a 43,870-byte cacheable chunk; the largest JavaScript chunk is 463,534 bytes, leaving 36,466 bytes under the hard gate.
...
- Current local `d941cad` is a fast-forward 162 commits beyond remote PR #258 head `338b6fd` and 208 beyond `main` `6885c320`; existing green checks cover only the remote head. The external mapping audit is anchored at `49b4e0e`; its later local checkpoints are not live.
...
- Live GitHub `main` is 208 commits behind the accepted local product checkpoint, while draft PR #258 is 162 commits behind it. The live app cannot contain the validated local work until owner-approved integration and a later coordinated release.
...
1. Audit Cancel from Ecommerce order review at 390/1280 px; preserve the prepared draft, Payment, source guard, focus return, and all Shop records.
