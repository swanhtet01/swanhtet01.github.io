...
- Checkpoint `37b9004` is the accepted local product and database-authority baseline.
...
- HQ remains its own route chunk; the largest JavaScript chunk is 495,689 bytes, leaving 4,311 bytes under the hard gate.
- Focused app-build coverage includes 160 Shop, 224 Plant, 94 Website, 11 managed-Website, 27 storefront-draft, 14 storefront, 15 request, 16 managed-storefront, and 16 Ecommerce handoff checks.
...
- Local `37b9004` is +154 over cached `origin/main` `6885c320` and +108 over cached release-candidate tracking `338b6fd`. Live GitHub/Vercel state was not refreshed; no push, merge, or deployment occurred.
...
- Cached GitHub `main` is 154 commits behind this product checkpoint. The live app cannot contain the validated local work until an owner-approved integration and coordinated release.
...
2. Review the 154-commit release-candidate delta, then obtain explicit owner approval before any push, merge, deployment, or canonical-domain correction.
