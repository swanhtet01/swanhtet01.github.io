...
`npm run qa:routes` passes the committed regression and usability policy across 14 public/app routes at 390 x 844 and 1280 x 900: 28 checks, zero horizontal overflow, zero unnamed visible actions, zero duplicate IDs, zero placeholder links, zero mobile touch targets below 44 px, zero console errors, zero page errors, zero external requests, and zero disclosure interaction failures. It also passes twelve browser workflow gates with 90 deterministic checkpoints: the seven-step Ecommerce-to-Shop journey, the ten-step Shop counter-sale-to-daily-close journey, the six-step Shop interrupted-write-to-recovery journey, the eight-step Plant plan-to-output-to-quality-close journey, the eight-step Website brief-to-responsive-preview-to-review-file journey, and the six-step shared Blueprint no-write data preview, each at mobile and desktop. `simplicityTargetsMet` is true, so the stricter `npm run qa:routes:strict` gate is now expected to pass.
