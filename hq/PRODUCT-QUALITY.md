...
`npm run qa:routes` passes the committed regression and usability policy across 14 public/app routes at 390 x 844 and 1280 x 900: 28 checks, zero horizontal overflow, zero unnamed visible actions, zero duplicate IDs, zero placeholder links, zero mobile touch targets below 44 px, zero console errors, zero page errors, zero external requests, and zero disclosure interaction failures. It also passes ten browser workflow gates with 78 deterministic checkpoints: the seven-step Ecommerce-to-Shop journey, the ten-step Shop counter-sale-to-daily-close journey, the eight-step Plant plan-to-output-to-quality-close journey, the eight-step Website brief-to-responsive-preview-to-review-file journey, and the six-step shared Blueprint no-write data preview, each at mobile and desktop. `simplicityTargetsMet` is true, so the stricter `npm run qa:routes:strict` gate is now expected to pass.
...
Open a useful sample workspace before asking a prospect to configure anything, then let an operator review sample or imported data before an accountable write. Ecommerce-to-Shop, Shop sale-to-close, Plant plan-to-quality-close, Website brief-to-review-file, and the existing Blueprint no-write preview are now proven at both viewports. Preserve these action-density ceilings and keep Blueprint inside each product instead of adding another customer page.

The next product slice is managed persistence and security: tenant isolation, idempotent sync/outbox behavior, recovery rehearsal, and hosted activation evidence. Do not add a fifth customer product, another setup shell, or a component-framework migration while those production boundaries remain open.

## Reproduce

Run `npm run qa:routes`. A full run includes all ten workflow gates; use `node tools/audit_product_routes.mjs --workflow` for the workflows alone or `node tools/audit_product_routes.mjs --route <route-id> --viewport mobile --details` for one route. Set `SUPERMEGA_QA_BROWSER_PATH` only when Edge, Chrome, or Chromium is not in a supported system location. The runner uses Playwright Core and the installed system browser, starts one local static server at a time, launches one browser, blocks external requests, and writes no report unless an explicit unused `--out` path is supplied.
