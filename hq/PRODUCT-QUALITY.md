...
`npm run qa:routes` passes the committed regression and usability policy across 14 public/app routes at 390 x 844 and 1280 x 900: 28 checks, zero horizontal overflow, zero unnamed visible actions, zero duplicate IDs, zero placeholder links, zero mobile touch targets below 44 px, zero console errors, zero page errors, zero external requests, and zero disclosure interaction failures. It also passes five browser workflow gates with 43 deterministic checkpoints: the seven-step mobile Ecommerce-to-Shop journey, the ten-step Shop counter-sale-to-daily-close journey at mobile and desktop, and the eight-step Plant plan-to-output-to-quality-close journey at mobile and desktop. `simplicityTargetsMet` is true, so the stricter `npm run qa:routes:strict` gate is now expected to pass.
...
Open a useful sample workspace before asking a prospect to configure anything, then let an operator review sample or imported data before an accountable write. Ecommerce-to-Shop is proven on mobile, while Shop sale-to-close and Plant plan-to-quality-close are proven at both viewports. Preserve the action-density ceilings while proving Website edit-to-release-review at both viewports, then add the Ecommerce journey at desktop.
