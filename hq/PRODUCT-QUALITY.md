...
`npm run qa:routes` passes the committed regression and usability policy across 14 public/app routes at 390 x 844 and 1280 x 900: 28 checks, zero horizontal overflow, zero unnamed visible actions, zero duplicate IDs, zero placeholder links, zero mobile touch targets below 44 px, zero console errors, zero page errors, zero external requests, and zero disclosure interaction failures. It also passes fourteen browser workflow gates with 98 deterministic checkpoints: the four-product onboarding-to-first-task journey, the seven-step Ecommerce-to-Shop journey, the ten-step Shop counter-sale-to-daily-close journey, the six-step Shop interrupted-write-to-recovery journey, the eight-step Plant plan-to-output-to-quality-close journey, the eight-step Website brief-to-responsive-preview-to-review-file journey, and the six-step shared Blueprint no-write data preview, each at mobile and desktop. `simplicityTargetsMet` is true, so the stricter `npm run qa:routes:strict` gate is now expected to pass.
...
- Shop, Plant, Website, and Ecommerce now prove that a fresh product setup has one required business-name field, keeps its primary action disabled until named, opens the correct separate product route, exposes one useful first action, and records setup, chosen outcome, and product-open events in order at both viewports.
- First-value measurement now starts from the latest onboarding route for that product and ignores older completions. Reopening Plant setup after an old completed trial correctly reports the new journey as in progress instead of instant success.
...
- Shop now stages each reviewed local action in a native IndexedDB write-ahead outbox before changing its local workspace. The browser proof interrupts acknowledgement after one approved order, confirms the durable intent remains, reloads, reconciles it to one recovery receipt, and verifies the order was not duplicated at either viewport. Conflicting state fails closed without an overwrite. This proof covers reviewed Shop actions routed through the commerce workspace; it does not authorize browser-to-managed replay or cover internal seed/import writers.
...
- Blueprint now proves the existing Shop data tool is reachable from the real product, matches a two-row sample locally, keeps approval unchecked and activation locked, leaves all four product stores unchanged during review, and makes zero external requests at both viewports. The deterministic launch rehearsal separately proves four-product staging, repair, digest binding, and `staged_not_applied` authority without product, hosted, connector, or model writes.

## Next product slice

Open a useful sample workspace before asking a prospect to configure anything, then let an operator review sample or imported data before an accountable write. Ecommerce-to-Shop, Shop sale-to-close, Shop local write recovery, Plant plan-to-quality-close, Website brief-to-review-file, and the existing Blueprint no-write preview are now proven at both viewports. Preserve these action-density ceilings and keep Blueprint and Sync inside each product instead of adding another customer page.

The next product slice is managed persistence and security: tenant isolation, tenant-bound idempotency, conflict handling, backup/restore rehearsal, and hosted activation evidence. Do not auto-replay browser-stored outbox state into a managed tenant; authenticated server authority and attributed review must own that transition. Do not add a fifth customer product, another setup shell, or a component-framework migration while those production boundaries remain open.

## Reproduce

Run `npm run qa:routes`. A full run includes all fourteen workflow gates; use `node tools/audit_product_routes.mjs --workflow` for the workflows alone or `node tools/audit_product_routes.mjs --route <route-id> --viewport mobile --details` for one route. Set `SUPERMEGA_QA_BROWSER_PATH` only when Edge, Chrome, or Chromium is not in a supported system location. The runner uses Playwright Core and the installed system browser, starts one local static server at a time, launches one browser, blocks external requests, and writes no report unless an explicit unused `--out` path is supplied.
