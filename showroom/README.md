...
This package is the canonical application for `app.supermega.dev`. It is one compact company system: Home, Work, and Products, with Settings as a utility. It is not the public marketing site, an agent showcase, or a client-specific deployment.
...
- `src/core/CoreApp.tsx` - shell, Home, Work, Commerce, Production, settings, and shared primitives; it consumes the root `site-manifest.json` workflow profiles directly.
