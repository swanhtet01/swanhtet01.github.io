...
export function isSharedAppHost(hostname = currentHostname()) {
  const normalized = normalizeHostname(hostname)
  return (
    normalized === supermegaAppHost ||
    normalized === `www.${supermegaAppHost}` ||
    normalized === supermegaPosHost ||
    normalized === `www.${supermegaPosHost}`
  )
}
...
export function getSupermegaHostSurface(hostname = currentHostname()): SupermegaHostSurface {
  ...
  if (isYtfClientHost(hostname)) {
    return 'ytf-client'
  }
  if (isSharedAppHost(hostname)) {
    return 'shared-app'
  }
  if (isPublicMarketingHost(hostname)) {
    return 'public-site'
  }
  return 'preview'
}
