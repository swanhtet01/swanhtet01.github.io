...
export function LeadPipelinePage() {
  ...
  const [searchName, setSearchName] = useState('Yangon spa search')
  const [searchQuery, setSearchQuery] = useState('spa in yangon')
  const [searchKeywords, setSearchKeywords] = useState('spa,wellness,massage,yangon')
  ...
  const loadData = useCallback(async () => {
    ...
    if (!session.authenticated) {
      throw new Error('Login is required to open the saved lead pipeline.')
    }

    const [pipelinePayload, huntPayload] = await Promise.all([
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) {
          setError(nextError instanceof Error ? nextError.message : 'Could not load the lead pipeline.')
        }
      } finally {
        if (!cancelled) {
          setLoading(false)
  ...
  const stageCounts = useMemo(
    () => ({
      ...
      contacted: pipeline?.summary.by_stage.contacted ?? 0,
  ...
  async function saveSearchProfile() {
    ...
    try {
      ...
      setMessage('Saved this search. Use Run to refresh the pipeline.')
      await loadData()
    } catch (nextError) {
  ...
  async function copyOutreach(row: LeadPipelineRow) {
    await navigator.clipboard.writeText(`${row.outreach_subject}\n\n${row.outreach_message}`)
    setMessage(`Copied outreach for ${row.company_name}.`)
  }

  if (loading) {
    return <section className="sm-surface p-6 text-sm text-[var(--sm-muted)]">Loading saved pipeline...</section>
  }
  ...
  return (
    <div className="space-y-8">
      <PageIntro
        eyebrow="Pipeline"
        title="Run the saved lead pipeline."
        description="Use one search box, one saved pipeline, and one follow-up path."
      />

      <section className="grid gap-4 md:grid-cols-4">
        <div className="sm-chip text-white">
          <p className="sm-kicker text-[var(--sm-accent)]">Total leads</p>
          <p className="mt-2 text-3xl font-bold">{pipeline?.summary.lead_count ?? 0}</p>
        </div>
        <div className="sm-chip text-white">
        ...
        <div className="sm-chip text-white">
          <p className="sm-kicker text-[var(--sm-accent-alt)]">Saved searches</p>
      ...
      <section className="grid gap-6 lg:grid-cols-[0.78fr_1.22fr]">
        <article className="sm-surface p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="sm-kicker text-[var(--sm-accent)]">Saved searches</p>
              <h2 className="mt-2 text-2xl font-bold text-white">Keep one market search running.</h2>
            </div>
          ...
          <div className="mt-5 space-y-3">
            {hunts.length ? (
              ...
              <div className="sm-chip text-[var(--sm-muted)]">No saved searches yet. Save one search so the pipeline can be refreshed in one click.</div>
            )}
          </div>
        </article>
        ...
        <article className="sm-terminal p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-bold text-white">Work the shortlist.</h2>
            </div>
          ...
          <div className="mt-5 space-y-4">
            {pipeline?.rows?.length ? (
              ...
              <div className="sm-chip text-[var(--sm-muted)]">No saved leads yet. Run a search and save the right ones into the pipeline.</div>
            )}
          </div>
        </article>
      </section>
