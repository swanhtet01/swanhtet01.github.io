    ...
    <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
      ...
      <article className="sm-surface-deep p-6"><div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/10 pb-5"><div><p className="sm-kicker text-[var(--sm-accent)]">2. Run the worker</p><h2 className="mt-2 text-2xl font-bold text-white">One order, four accountable states</h2></div><span className="sm-status-pill">{businessName} / {channel}</span></div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
          <div><p className="text-sm text-[var(--sm-muted)]">Customer message</p><div className="mt-3 space-y-3 text-sm leading-relaxed"><div className="ml-auto max-w-[85%] bg-white/10 p-4 text-white">Hello, do you have milk tea? I want two cups for delivery.</div><div className="max-w-[85%] border border-[var(--sm-accent)]/30 bg-[var(--sm-accent)]/10 p-4 text-white">Yes. I found {selectedProduct.name} and can prepare an order for review.</div></div><div className="mt-4 flex flex-wrap gap-2"><span className="sm-chip text-white">Intent / ready to buy</span><span className="sm-chip text-white">Language / customer channel</span><span className="sm-chip text-white">Confidence / 94%</span></div></div>
        ...
        <div className="mt-6 border-t border-white/10 pt-5 text-sm text-[var(--sm-muted)]"><p>Current state: <strong className="capitalize text-white">{stage}</strong></p><p className="mt-2">Next action: <span className="text-white">{stage === 'handoff' ? 'Operator confirms delivery details and payment evidence.' : 'Advance the order when the preceding evidence is present.'}</span></p></div>
