...
import { catalogExport, loadShopCatalog, normalizeCommerceCatalogText, normalizeCommerceMachineBlueprint, type CommerceCatalogSource, type CommerceCatalogItem, type CommerceMachinePolicy } from '../lib/commerceMachineAdapter'
  ...
  function importCatalog(file: File) {
    ...
    reader.onload = () => {
      try {
        ...
        const csv = normalizeCommerceCatalogText(String(reader.result))
        if (csv.length) { setProducts(csv); setSelectedProductId(csv[0].id); setCatalogSource('client-import'); setStage('draft'); trackEvent('commerce_catalog_imported', { item_count: csv.length, source: 'client_csv' }); return }
        window.alert(error instanceof Error ? error.message : 'Could not import catalog.')
      }
    }
  ...
  return <div className="space-y-8 pb-12">
    ...
    <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
      <article className="sm-terminal p-6"><p className="sm-kicker text-[var(--sm-accent)]">1. Configure</p><h2 className="mt-2 text-2xl font-bold text-white">Business template</h2>
        ...
        <label className="mt-6 block cursor-pointer border border-dashed border-white/20 p-4 text-sm text-[var(--sm-muted)] hover:border-[var(--sm-accent)]"><span className="block font-semibold text-white">Import client catalog or blueprint</span><span className="mt-1 block">Upload JSON, CSV, or a saved SuperMega machine blueprint.</span><input className="sr-only" accept="application/json,.json,text/csv,.csv" type="file" onChange={(event) => { const file = event.target.files?.[0]; if (file) importCatalog(file); event.currentTarget.value = '' }} /></label>
      ...
      <article className="sm-surface-deep p-6"><div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/10 pb-5"><div><p className="sm-kicker text-[var(--sm-accent)]">2. Run the worker</p><h2 className="mt-2 text-2xl font-bold text-white">One order, four accountable states</h2></div><span className="sm-status-pill">{businessName} / {channel}</span></div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.9fr]">
          ...
          <div><label className="block space-y-2 text-sm text-[var(--sm-muted)]"><span>Recommended item</span><select className="sm-input" value={selectedProduct.id} onChange={(event) => { setSelectedProductId(event.target.value); setConversationReady(false); setParseNote('Item changed. Parse the customer message again to confirm the new draft.') }}>{products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label><label className="mt-4 block space-y-2 text-sm text-[var(--sm-muted)]"><span>Quantity</span><input className="sm-input" min={1} max={selectedProduct.stock} type="number" value={quantity} onChange={(event) => { setQuantity(Math.max(1, Number(event.target.value) || 1)); setConversationReady(false); setParseNote('Quantity changed. Parse the customer message again to confirm the draft.') }} /></label><div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="sm-chip text-white"><span className="text-[var(--sm-muted)]">Order total</span><strong className="mt-1 block text-xl">{money(total)}</strong></div><div className="sm-chip text-white"><span className="text-[var(--sm-muted)]">Stock after draft</span><strong className={`mt-1 block text-xl ${stockAfterDraft < 3 ? 'text-amber-300' : 'text-emerald-300'}`}>{stockAfterDraft} {selectedProduct.unit}s</strong></div></div></div>
        </div>
