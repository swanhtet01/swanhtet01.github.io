...
export type EcommerceRescheduleIntentFulfilmentOrderStatusBrief = {
  totalIntents: number
...
export function projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief(
  buying: EcommerceBuyingState,
): EcommerceRescheduleIntentFulfilmentOrderStatusBrief {
  ...
  for (const intent of buying.rescheduleIntents) {
    const isDelivery = intent.fulfilment === 'delivery'
    if (isDelivery) {
