    ...
    <div className="workspace-screen settings-screen">
      ...
      <div className="settings-grid settings-step-content">
        <form className="core-panel setup-form" onSubmit={save}>
          <div className="panel-head"><div><span className="core-eyebrow">{requestedProduct ? 'One product' : 'Client demo'}</span><h2>{settingsStep === 'workflow' ? requestedProduct ? 'Choose the trial' : 'Client and business' : 'Define success'}</h2></div><span className={`status-pill ${displayedReady ? 'approved' : 'bounded'}`}>{displayedReady ? 'ready' : `${displayedCompletion}%`}</span></div>
