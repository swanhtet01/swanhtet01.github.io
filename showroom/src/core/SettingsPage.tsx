    ...
    <div className="workspace-screen settings-screen">
      ...
      <div className="settings-grid settings-step-content">
        <form className="core-panel setup-form" onSubmit={save}>
          ...
          {requestedProduct ? <div className="settings-step-actions"><span>{workflowReady ? `Opens a working ${selectedProduct.name} sample on this device.` : 'Enter the client name and responsible owner to open the sample.'}</span><div className="setup-action-group"><button className="text-link" disabled={!workflowReady} onClick={() => chooseSettingsStep('success')} type="button">Add success criteria</button><button className="core-button primary" disabled={!workflowReady} onClick={startGuidedTrial} type="button">Open working sample</button></div></div> : null}
