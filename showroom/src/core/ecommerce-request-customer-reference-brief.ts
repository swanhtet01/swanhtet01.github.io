...
export function projectEcommerceRequestCustomerReferenceBrief(
  buying: EcommerceBuyingState,
): EcommerceRequestCustomerReferenceBrief {
  const total = buying.requests.length
  if (total === 0)
    return { totalRequests: 0, uniqueCustomers: 0, topCustomer: null, topCustomerCount: 0 }
