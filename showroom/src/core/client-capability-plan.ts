...
const catalog: Record<ClientSolutionId, readonly Omit<ClientCapability, 'product'>[]> = {
  ...
  production: [
    ...
    { id: 'plant-maintenance', phase: 'prove', delivery: 'demo', domain: 'operations', label: 'Machine, downtime, and maintenance', outcome: 'Record machine state, downtime, maintenance work, return-to-service, and shift handoff.', records: ['machine', 'downtime', 'maintenance_order', 'shift_handoff'], roles: ['Maintenance technician', 'Shift supervisor'], dependsOn: ['plant-job-control'], proofPath: '/plant/?tab=maintenance' },
