...
export type EcommerceRescheduleIntentCustomerMessageFulfilmentBrief = {
  totalIntents: number
...
export function projectEcommerceRescheduleIntentCustomerMessageFulfilmentBrief(
  buying: EcommerceBuyingState,
): EcommerceRescheduleIntentCustomerMessageFulfilmentBrief {
  ...
  for (const intent of buying.rescheduleIntents) {
    if (intent.customerMessageSent) {
      if (intent.fulfilment === 'delivery') messageSentDeliveryCount++
      else messageSentPickupCount++
    } else {
      if (intent.fulfilment === 'delivery') noMessageDeliveryCount++
