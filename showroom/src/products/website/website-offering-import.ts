...
export function previewWebsiteOfferingCsv(input: string): WebsiteOfferingRow[] {
  ...
  return rows.slice(1).map(row => {
    ...
    if (!name || name.length > 80 || name.includes('|') || !details || details.length > 360 || /[\u0000-\u001f\u007f]/u.test(name + details)) throw new Error(`Row ${row.rowNumber}: use a name of 1–80 characters without | and a description of 1–360 characters.`)
