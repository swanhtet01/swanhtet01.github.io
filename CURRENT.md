...
2. **Website** — finite page/content workflow, responsive preview, attributed evidence, evidence-bound human approval, and an approved local snapshot; currently local prototype only.
...
- `/products/website/` — **Website**: lazy-loaded local Content, Navigation, and Publish workflow with responsive preview, revisioned evidence, approval, and snapshot records.
...
- Team, bounded-agent, Website, Commerce, Production, approval, and setup records can be exercised locally. Website explicitly remains a browser-local prototype; core evidence export v9 includes the team roster and operating records while preserving the isolated-demo boundary.
- Website workspace v2 deterministically migrates valid v1 records without inventing events or reopening historical approvals. A Web Lock serializes reread, transition, validation, write, and readback confirmation. Content revisions prevent stale page edits; independent evidence actions can merge while content is unchanged. Evidence, approval, and approved snapshots retain append-only human-attributed events with exact source and evidence IDs. Malformed newer data, duplicate or blank proof, stale content, unavailable locks, and unconfirmed storage writes fail closed without replacing the stored record.
