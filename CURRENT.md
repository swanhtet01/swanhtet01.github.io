...
- **Shop** — available at `/shop/` in the live isolated app under the stable internal `commerce` runtime. First-run setup accepts reviewed product CSV rows before opening Sell; new real orders also require one reviewed business and operating location. Orders freeze that scope, same-day closes and account mappings stay separate by location, and mixed books fail closed. Optional industry demos remain explicit; changed records are preserved fail-closed. `/operations/commerce/` is compatibility-only and resolves to the same records.
- **Plant** — available at `/plant/` in the live isolated app under the stable internal `production` runtime. First-run setup now accepts reviewed job CSV rows before opening Jobs; an exact untouched built-in or selected industry sample is replaced, changed workspaces remain protected, and optional industry demos stay explicit. Browser-local business jobs render as a local production plan rather than sample data. `/operations/production/` is compatibility-only and resolves to the same records.
...
- Shop and Plant currently use the stable `commerce` and `production` state contracts; renaming the interface must not migrate, fork, or silently reset those records. Shop business/location reviews are append-only, and each new real order retains the exact reviewed scope used by its close and accounting export.
