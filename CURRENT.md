...
- `/products/website/` — **Website**: mobile opens one task-first editor with Edit and Publish, a compact page/Preview row, and page, navigation, or search controls inside Site settings; desktop retains Pages, Navigation, Publish, and Split preview. Revisioned evidence, approval, and a retained self-contained HTML site file remain unchanged; authenticated v3 workspaces sync through the managed API and all other sessions remain local or fail closed.
