...
- The Website-to-Ecommerce handoff carries only an approved local revision reference, SKU, and quantity. Ecommerce revalidates the source, requires a bounded human operator ID, and atomically records one accepted local intake plus its audit event; it does not create an order or handle customer data.
