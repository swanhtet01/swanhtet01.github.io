# Website — client demo playbook

## 1. Client and the 30-second pitch

Who this is for: a company that needs a credible web presence without an agency — the manifest ships the templates `Business presence`, `Lead generation`, and `Catalog showcase`, entered from a `Business brief`, an `Existing website`, a `Company profile`, a `Campaign brief`, or a `Catalog CSV`.
