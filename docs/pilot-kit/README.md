...
- The baseline form can be converted into a public-safe digest packet with `npm.cmd run shop:pilot:baseline-packet -- --input <private-baseline.json> --output <public-baseline-packet.json>`. The Shop launch gate can then read that packet with `npm.cmd run shop:pilot:launch-gate:verify -- --baseline-packet <public-baseline-packet.json>` and still refuses contact, payment, deployment, hosted writes, and managed activation.
