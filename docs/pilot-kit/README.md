...
- The baseline form can be converted into a public-safe digest packet with the `shop:pilot:baseline-packet` script. The Shop launch gate can then read that packet with `shop:pilot:launch-gate:verify` and still refuses contact, payment, deployment, hosted writes, and managed activation.
