...
class TrialStoreTests(unittest.TestCase):
    ...
    def test_state_and_command_ids_are_scoped_to_workspace(self) -> None:
        ...
        self.assertEqual(self.store.get_state(self.operator, "shop").state["sku"], "workspace-a-sku")
        self.assertEqual(
            self.store.get_state(self.other_operator, "shop").state["sku"],
            "workspace-b-sku",
        )
        self.assertEqual(self.store.get_state(self.operator, "shop").updated_by, "actor-operator")
    ...
    def test_optimistic_version_conflict_does_not_change_state(self) -> None:
        ...
        self.assertEqual(self.store.get_state(self.operator, "shop").state["sku"], "sku-a")
    ...
    def test_write_requires_explicit_surface_capability(self) -> None:
        ...
        self.assertEqual(raised.exception.required_capability, "plant.write")
        self.assertEqual(self.store.get_state(self.operator, "plant").version, 0)
    ...
    def test_auth_and_active_membership_are_required_for_writes(self) -> None:
        unauthenticated = TrialPrincipal("workspace-a", "actor-operator", authenticated=False)
        ...
        missing = TrialPrincipal("workspace-a", "missing-actor")
        ...
        suspended = TrialPrincipal("workspace-a", "suspended-actor")
    ...
    def test_reads_do_not_bypass_database_schema_auth_or_membership(self) -> None:
        for blocked_check in ("database_ready", "role_ready", "schema_ready"):
            with self.subTest(blocked_check=blocked_check):
                ...
                with self.assertRaises(TrialNotReadyError):
                    store.get_state(self.operator, "shop")
        ...
        self.assertEqual(read_only.get_state(self.operator, "shop").version, 0)
    ...
    def test_approval_transition_is_controlled_and_workspace_scoped(self) -> None:
        ...
        approval.proposal["release"] = "client-tamper"
        ...
        self.assertEqual(stored.proposal["release"], "catalog-v1")
