...
function proofReviewNextGate(decision) {
  if (decision === 'ready_for_paid_pilot') return 'scope_price_owner_approval_required'
  if (decision === 'changes_requested') return 'proof_revision_required'
  return 'discovery_rework_required'
}

function proofReviewNextStep(decision) {
  if (decision === 'ready_for_paid_pilot') return 'Prepare owner-approved scope and price, then payment route for the paid pilot.'
  if (decision === 'changes_requested') return 'Revise the first proof from the requested changes before scope and price.'
  return 'Run a smaller discovery pass before making a paid-pilot offer.'
}

function normalizeProofReviewSubmission(payload = {}) {
  const imported = parseJsonObject(
    payload.proof_acceptance_json ||
      payload.proof_review_acceptance_json ||
      payload.acceptance_json ||
      payload.proof_review_acceptance ||
      payload,
  )
  ...
  const decision = truncate(merged.decision || merged.review_decision, 80)
  if (!['ready_for_paid_pilot', 'changes_requested', 'not_useful'].includes(decision)) {
    return { status: 'error', reason: 'invalid_proof_review_decision' }
  }

  const recordedAt = new Date().toISOString()
  const clientNote = truncate(merged.client_note || merged.note, 1600)
  const proofPacketExcerpt = truncate(merged.proof_packet_excerpt || merged.proof_excerpt, 1800)
  const nextGate = proofReviewNextGate(decision)
  const packet = [
    '# AI Workcell proof review acceptance',
    '',
    `Decision: ${decision}`,
    `Lead: ${leadId}`,
    `Action: ${actionId}`,
  ...
  return {
    ...
    submission_type: 'client_proof_review_submission',
    lead_id: leadId,
    action_id: actionId,
    decision,
    ...
    next_gate: nextGate,
    next_step: proofReviewNextStep(decision),
    ...
    recorded_by: 'client_proof_review_room',
    packet,
    guardrails: [
      'client_review_submission_only',
      'owner_scope_price_approval_required',
      'no_external_send',
      'no_connector_write',
      'no_browser_action',
      'no_payment_request',
...
function publicSubmissionResponse({ submission, action, adapter, primaryDatabase }) {
  return {
    status: 'ready',
    endpoint: 'proof-review-submissions',
    adapter: adapter || null,
    primary_database: primaryDatabase || null,
    submission_status: 'proof_review_acceptance_received',
    action: {
      action_id: text(action.action_id || submission.action_id) || null,
      lead_id: text(action.lead_id || submission.lead_id) || null,
      status: text(action.status) || proofReviewActionStatus(submission.decision),
      approval_state: text(action.approval_state) || 'pending_owner_review',
      notification_status: text(action.notification_status) || 'proof_review_acceptance_received',
    ...
    proof_review_acceptance: {
      ...
      workspace_state: submission.workspace_state,
      ...
      proof_packet_excerpt_received: Boolean(submission.proof_packet_excerpt),
    },
  }
}
...
async function recordBlobSubmission(submission, primaryDatabase = {}) {
  ...
  const nextResult = {
    ...currentResult,
    proof_review_acceptance: submission,
    proof_review_acceptance_recorded_at: submission.recorded_at,
  }
  const updated = await blobQueue.updateActionRecord(
    { action_id: submission.action_id, lead_id: submission.lead_id },
    {
      result: nextResult,
      status: proofReviewActionStatus(submission.decision),
...
async function recordSubmission(submission) {
  ...
  const updateResultExpression = resultColumn.has_result
    ? "result = coalesce(result, '{}'::jsonb) || jsonb_build_object('proof_review_acceptance', $2::jsonb, 'proof_review_acceptance_recorded_at', $3::text),"
    : ''
  ...
  const updated = await datastore.query(
    `
      ...
      set
        ...
        payload = coalesce(payload, '{}'::jsonb) || jsonb_build_object(
          'proof_review_acceptance', $2::jsonb,
          'proof_review_acceptance_recorded_at', $3::text
        ),
        status = $4,
    ...
    [
      ...
      proofReviewActionStatus(submission.decision),
...
function writeStatusCode(result) {
  ...
  if (['missing_action_or_lead_id', 'invalid_proof_review_decision', 'invalid_json', 'request_too_large'].includes(result.reason)) return 400
  return 500
}
...
async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    json(res, 200, {
      ...
      allowed_decisions: ['ready_for_paid_pilot', 'changes_requested', 'not_useful'],
  ...
  const submission = normalizeProofReviewSubmission(payload)
