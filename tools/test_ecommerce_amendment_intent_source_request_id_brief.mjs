...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentSourceRequestIdBrief } from './ecommerce-amendment-intent-source-request-id-brief.ts'`,
...
const { projectEcommerceAmendmentIntentSourceRequestIdBrief } = await import(
...
let intentId = 0
function amendmentIntent(sourceRequestId = 'REQ-DEFAULT') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceAmendmentIntentSourceRequestIdBrief(state())
...
{
  const r = projectEcommerceAmendmentIntentSourceRequestIdBrief(state([amendmentIntent('REQ-001')]))
...
{
  const r = projectEcommerceAmendmentIntentSourceRequestIdBrief(state([
    amendmentIntent('REQ-003'),
    amendmentIntent('REQ-004'),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentSourceRequestIdBrief(state([
    amendmentIntent('REQ-005'),
    amendmentIntent('REQ-006'),
    amendmentIntent('REQ-005'),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentSourceRequestIdBrief(state([
    amendmentIntent('REQ-007'),
    amendmentIntent('REQ-008'),
    amendmentIntent('REQ-009'),
  ]))
...
console.log(`ecommerce-amendment-intent-source-request-id-brief: ${checks} checks passed`)
