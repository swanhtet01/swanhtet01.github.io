...
test('builds an exact non-mutating v11 rehearsal packet', async () => {
  ...
  assert.equal(packet.release.schemaVersion, 11)
  assert.equal(packet.release.migrationCount, 12)
  assert.deepEqual(packet.release.review, releaseReview)
  assert.equal(packet.release.migrations.at(-1).name, '20260816120000_private_trial_backend_v11_self_serve_grants.sql')
