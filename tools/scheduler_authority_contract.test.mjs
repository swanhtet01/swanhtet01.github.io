...
test('dormant scheduler authority has one two-job activation budget', () => {
  ...
  assert.equal(result.maxClaimsPerInvocation, 2)
...
test('scheduler budget rejects active, wider, or higher-frequency authority', () => {
  const cases = [
    ...
    (value) => { value.activation_plan.crons[0].max_claims_per_invocation = 3 },
