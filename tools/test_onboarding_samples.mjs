...
check(spaWebsite.sampleCsv.includes('Ask about treatment duration, suitability, price and cancellation terms.'), 'the Beauty & spa Website gives trade-specific inquiry guidance')
check(spaWebsite.sampleCsv.includes('an inquiry is not a confirmed appointment'), 'the Beauty & spa Website distinguishes inquiries from booked capacity')
check(!spaWebsite.sampleCsv.includes('Every treatment is held against a named therapist and time'), 'the Beauty & spa sample does not invent retained therapist reservations')
