...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceCancellationIntentCustomerMessageSentRatesBrief } from './ecommerce-cancellation-intent-customer-message-sent-rates-brief.ts'`,
...
const { projectEcommerceCancellationIntentCustomerMessageSentRatesBrief } = await import(
...
let intentId = 0
function cancellationIntent({ customerMessageSent = false } = {}) {
  intentId++
  return {
...
{
  const r = projectEcommerceCancellationIntentCustomerMessageSentRatesBrief(state())
...
{
  const r = projectEcommerceCancellationIntentCustomerMessageSentRatesBrief(state([cancellationIntent()]))
...
console.log(`ecommerce-cancellation-intent-customer-message-sent-rates-brief: ${checks} checks passed`)
