...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceCorrectionIntentRefundStartedRatesBrief } from './ecommerce-correction-intent-refund-started-rates-brief.ts'`,
...
const { projectEcommerceCorrectionIntentRefundStartedRatesBrief } = await import(
...
let intentId = 0
function correctionIntent({ refundStarted = false } = {}) {
  intentId++
  return {
...
{
  const r = projectEcommerceCorrectionIntentRefundStartedRatesBrief(state())
...
{
  const r = projectEcommerceCorrectionIntentRefundStartedRatesBrief(state([correctionIntent()]))
...
console.log(`ecommerce-correction-intent-refund-started-rates-brief: ${checks} checks passed`)
