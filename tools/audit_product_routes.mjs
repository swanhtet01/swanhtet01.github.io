...
async function auditShopSaleCloseWorkflow(browser, baseUrl, viewport) {
  ...
  try {
    ...
    await page.waitForURL((url) => url.pathname === '/shop/' && url.searchParams.get('tab') === 'orders' && url.hash.startsWith('#shop-order-'), { timeout: 8_000 })
    const createdOrderTarget = await expectOneVisible(page.locator('.order-list article[data-highlighted="true"]').filter({ hasText: customer }), 'workflow_shop_created_order_target_count')
    if (await createdOrderTarget.getAttribute('id') !== new URL(page.url()).hash.slice(1)) fail('workflow_shop_created_order_target_mismatch')
    checkpoints.push('created_order_handoff_focused')
