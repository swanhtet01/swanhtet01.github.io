import { existsSync, readFileSync } from 'node:fs'
import { resolve } from 'node:path'

function fail(reason, detail = {}) {
  console.error(JSON.stringify({ status: 'error', reason, ...detail }, null, 2))
  process.exit(1)
}

const root = process.cwd()
const generatorPath = resolve(root, 'tools/create_public_vercel_output.mjs')
const operatorPath = resolve(root, '.vercel/output/static/operator/index.html')

if (!existsSync(generatorPath)) fail('public_generator_missing')
if (!existsSync(operatorPath)) fail('operator_output_missing', { next: 'Run node tools/create_public_vercel_output.mjs first.' })

const generator = readFileSync(generatorPath, 'utf8')
const operator = readFileSync(operatorPath, 'utf8')

const sourceTokens = [
  'recordScopePriceApproval',
  'data-scope-price-command',
  'approved_scope_summary',
  'approved_price_mmk',
  'owner_approval_reference',
  'Record scope + payment approval',
  'scope_price_approval_reference_required',
  "operation:'record_scope_price_approval'",
]

for (const token of sourceTokens) {
  if (!generator.includes(token)) fail('scope_price_approval_source_token_missing', { token })
}

const outputTokens = [
  'Record scope + payment approval',
  'Approved pilot scope',
  'Approved price MMK',
  'Owner approval reference',
  'Payment request gate',
  'Copy payment request gate',
  'data-scope-price-command',
  'approved_price_mmk',
  'owner_approval_reference',
]

for (const token of outputTokens) {
  if (!operator.includes(token)) fail('scope_price_approval_output_token_missing', { token })
}

console.log(
  JSON.stringify(
    {
      status: 'ready',
      audit: 'operator-scope-price-approval-contract',
      source_tokens: sourceTokens.length,
      output_tokens: outputTokens.length,
    },
    null,
    2,
  ),
)
