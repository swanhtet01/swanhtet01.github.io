...
test('template is blank and CLI generates metadata-only packet output', async () => {
  ...
  try {
    ...
    const preflightPath = join(parent, 'baseline-preflight.json')
    ...
    assert.equal(lintedResult.status, 'baseline_input_ready')
    assert.equal(lintedResult.preflightOutputWritten, true)
    assert.equal(lintedResult.output, undefined)
    assert.doesNotMatch(linted.stdout, /Private Spa Sample|Private Operator/)
    const preflight = validateShopPilotBaselineInputPreflight(JSON.parse(await readFile(preflightPath, 'utf8')))
    assert.equal(preflight.status, 'baseline_input_ready')
    assert.doesNotMatch(JSON.stringify(preflight), /Private Spa Sample|Private Operator/)

    const verifiedPreflight = spawnSync(process.execPath, [tool, '--verify-preflight', preflightPath], { encoding: 'utf8' })
    assert.equal(verifiedPreflight.status, 0, verifiedPreflight.stderr)
    assert.equal(JSON.parse(verifiedPreflight.stdout).baselineInputReady, true)
    assert.doesNotMatch(verifiedPreflight.stdout, /Private Spa Sample|Private Operator/)

    const preflightOverwrite = spawnSync(process.execPath, [tool, '--lint-input', inputPath, '--output', preflightPath], { encoding: 'utf8' })
    assert.notEqual(preflightOverwrite.status, 0)
    assert.match(preflightOverwrite.stderr, /shop_pilot_baseline_output_exists/)

    const invalidInputPath = join(parent, 'invalid-baseline.private.json')
    const invalidPreflightPath = join(parent, 'invalid-baseline-preflight.json')
    await writeFile(invalidInputPath, `${JSON.stringify(input({ businessName: 'owner@example.invalid' }), null, 2)}\n`)
    const invalidLint = spawnSync(process.execPath, [tool, '--lint-input', invalidInputPath, '--output', invalidPreflightPath], { encoding: 'utf8' })
    assert.notEqual(invalidLint.status, 0)
    assert.equal(JSON.parse(invalidLint.stdout).status, 'baseline_input_invalid')
    assert.equal(validateShopPilotBaselineInputPreflight(JSON.parse(await readFile(invalidPreflightPath, 'utf8'))).status, 'baseline_input_invalid')
