...
for (const required of ['Plant agent queue', 'Recommended Plant agent job', 'Agent job', 'Owner gate', 'Record next job output', 'Contain urgent Plant problems', 'Review quality holds', 'Close WCM records', 'Build shift handoff', 'Plant control', 'Contain urgent problems', 'Close WCM work', 'MES lifecycle', 'Plan to handoff', 'AI guides plan, execution, quality, WCM, traceability, and handoff from one Plant record.', 'No equipment command or production write runs without owner approval.', 'Jobs', 'Quality', 'WCM', 'Materials', 'Handoff', 'Write gate', 'Plan', 'Execute', 'Trace', 'Owner confirms every production, quality, WCM, maintenance, material, and shift-handoff write before Plant changes.']) {
  if (!assetCorpus.includes(required)) throw new Error(`missing_live_plant_context:${required}`)
}
for (const required of ['Shop agent queue', 'Recommended Shop agent job', 'Agent job', 'Owner gate', 'Restore Shop write readiness', 'Review online order requests', 'Finish fulfilment queue', 'Receive purchase orders', 'Reorder low stock', 'Set up stock locations', 'Order control', 'Review Ecommerce inbox', 'Reconcile payment exceptions', 'Online inbox', 'Write gate', 'Owner confirms every order, payment, refund, delivery, cancellation, and stock change before Shop writes.', 'Shop order lifecycle', 'Order lifecycle', 'Capture to return', 'AI guides capture, reservation, fulfilment, collection, replenishment, and returns from one Shop record.', 'Owner confirms every order, payment, refund, delivery, cancellation, and stock write.', 'Capture', 'Reserve', 'Fulfil', 'Collect', 'Replenish', 'Return']) {
