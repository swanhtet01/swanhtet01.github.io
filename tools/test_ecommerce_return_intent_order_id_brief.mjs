...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceReturnIntentOrderIdBrief } from './ecommerce-return-intent-order-id-brief.ts'`,
...
const { projectEcommerceReturnIntentOrderIdBrief } = await import(
...
let intentId = 0
function returnIntent(orderId = 'ORD-DEFAULT') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceReturnIntentOrderIdBrief(state())
...
{
  const r = projectEcommerceReturnIntentOrderIdBrief(state([returnIntent('ORD-001')]))
...
{
  const r = projectEcommerceReturnIntentOrderIdBrief(state([
    returnIntent('ORD-003'),
    returnIntent('ORD-004'),
  ]))
...
{
  const r = projectEcommerceReturnIntentOrderIdBrief(state([
    returnIntent('ORD-005'),
    returnIntent('ORD-006'),
    returnIntent('ORD-005'),
  ]))
...
{
  const r = projectEcommerceReturnIntentOrderIdBrief(state([
    returnIntent('ORD-007'),
    returnIntent('ORD-008'),
    returnIntent('ORD-009'),
  ]))
...
console.log(`ecommerce-return-intent-order-id-brief: ${checks} checks passed`)
