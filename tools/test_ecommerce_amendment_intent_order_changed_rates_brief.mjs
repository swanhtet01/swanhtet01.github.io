...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentOrderChangedRatesBrief } from './ecommerce-amendment-intent-order-changed-rates-brief.ts'`,
...
const { projectEcommerceAmendmentIntentOrderChangedRatesBrief } = await import(
...
let intentId = 0
function amendmentIntent({ orderChanged = false } = {}) {
  intentId++
  return {
...
{
  const r = projectEcommerceAmendmentIntentOrderChangedRatesBrief(state())
...
{
  const r = projectEcommerceAmendmentIntentOrderChangedRatesBrief(state([
    amendmentIntent({ orderChanged: true }),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentOrderChangedRatesBrief(state([amendmentIntent()]))
...
console.log(`ecommerce-amendment-intent-order-changed-rates-brief: ${checks} checks passed`)
