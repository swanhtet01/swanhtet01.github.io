...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentOrderStatusRatesBrief } from './ecommerce-amendment-intent-order-status-rates-brief.ts'`,
...
const { projectEcommerceAmendmentIntentOrderStatusRatesBrief } = await import(
...
let intentId = 0
function amendmentIntent({ orderStatus = 'confirmed' } = {}) {
  intentId++
  return {
...
{
  const r = projectEcommerceAmendmentIntentOrderStatusRatesBrief(state())
...
{
  const r = projectEcommerceAmendmentIntentOrderStatusRatesBrief(state([
    amendmentIntent({ orderStatus: 'confirmed' }),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentOrderStatusRatesBrief(state([
    amendmentIntent({ orderStatus: 'preparing' }),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentOrderStatusRatesBrief(state([
    amendmentIntent({ orderStatus: 'ready' }),
  ]))
...
console.log(`ecommerce-amendment-intent-order-status-rates-brief: ${checks} checks passed`)
