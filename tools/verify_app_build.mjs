...
if (!ecommerceSource.includes('const orderAutopilotStage =')
  || !ecommerceSource.includes('const orderAutopilotNextAction =')
  || !ecommerceSource.includes('const orderAutopilotRows = [')
  || !ecommerceSource.includes('function runOrderAutopilot()')
  || !ecommerceSource.includes('Order Autopilot')
  || !ecommerceSource.includes('One button chooses the next safe owner action from catalog, storefront, order import, checkout, and Shop queue state.')
  || !ecommerceSource.includes('AI may prepare packets and drafts; it does not send customer messages, charge payments, book delivery, move stock, or write Shop orders here.')
  || !ecommerceSource.includes("['Learning', 'Records behavior only']")
  || !ecommerceSource.includes("['Boundary', 'No auto write']")
  ...
  || !ecommerceCssSource.includes('.ecommerce-order-command-center')
...
if (!productHomePageContract.includes('title="What needs attention?"')
  ...
  || !productHomePageContract.includes("'/ecommerce/'")
  || !productHomePageContract.includes('customerTracks.map(([name, fit, outcome, path, setupPath])')
...
if (!productHomeReadinessSource.includes('const launchReadinessRows = [')
  ...
  || !productHomeReadinessSource.includes('const [behaviorTrail] = useState(() => readBehaviorTrail(window.localStorage))')
  || productHomeReadinessSource.includes('setBehaviorTrail(readBehaviorTrail(window.localStorage))')
  || !productHomeReadinessSource.includes('const learningCockpitRows = [')
  || !productHomeReadinessSource.includes('const agentCommandQueueRows = [')
  || !productHomeReadinessSource.includes('const operatingTrackRows = [')
  || !productHomeReadinessSource.includes('const businessTrackRows = [')
  || !productHomeReadinessSource.includes('const trackRunbookRows = [')
  || !productHomeReadinessSource.includes('const trackActionRows = [')
  || !productHomeReadinessSource.includes('recordBehaviorSignal')
  || !productHomeReadinessSource.includes("function recordLaunchPackChoice(label: string, action: 'prepare data' | 'open workspace')")
  || !productHomeReadinessSource.includes("detail: `${label}: ${action}`")
  || !productHomeReadinessSource.includes("onClick={() => recordLaunchPackChoice(label, 'prepare data')}")
  || !productHomeReadinessSource.includes("onClick={() => recordLaunchPackChoice(label, 'open workspace')}")
  ...
  || !productHomeReadinessSource.includes('SuperMega ranks setup, import, product work, managed activation, and learning handoff into safe next actions.')
  ...
  || !productHomeReadinessSource.includes("ready ? 'Export evidence' : 'Finish setup'")
  || !productHomeReadinessSource.includes("ready ? '/settings/#controls' : '/settings/'")
  ...
  || !productHomeReadinessSource.includes('Enterprise autopilot')
  || !productHomeReadinessSource.includes('Minimal control, accountable execution.')
  || !productHomeReadinessSource.includes('Draft work automatically')
  || !productHomeReadinessSource.includes('Approval before impact')
  || !productHomeReadinessSource.includes('One control model')
  || !productHomeReadinessSource.includes('aria-label="Business starter tracks"')
  || !productHomeReadinessSource.includes('Business starter tracks')
  || !productHomeReadinessSource.includes('Pick the business type. SuperMega picks the modules.')
  || !productHomeReadinessSource.includes('Clients should not learn every enterprise feature on day one.')
  || !productHomeReadinessSource.includes('Retail or distributor')
  || !productHomeReadinessSource.includes('Factory or workshop')
  || !productHomeReadinessSource.includes('Service or local brand')
  || !productHomeReadinessSource.includes('Multi-branch operator')
  || !productHomeReadinessSource.includes('Start with product CSV, stock counts, online order CSV, and payment review proof.')
  || !productHomeReadinessSource.includes('Start with job CSV, material list, quality holds, maintenance evidence, and stock issue proof.')
  || !productHomeReadinessSource.includes('Start with business facts, offer proof, photos, catalog rows, and channel order samples.')
  || !productHomeReadinessSource.includes('Start with one branch pack, role list, import samples, approval receipts, and shared evidence export.')
  || !productHomeReadinessSource.includes('Free proof')
  || !productHomeReadinessSource.includes('Local enterprise max')
  || !productHomeReadinessSource.includes('Premium unlock')
  || !productHomeReadinessSource.includes('Managed AI context')
  || !productHomeReadinessSource.includes('First sale motion')
  || !productHomeReadinessSource.includes('Prepared template demo')
  || !productHomeReadinessSource.includes('Operator promise')
  || !productHomeReadinessSource.includes('Minimal controls')
  || !productHomeReadinessSource.includes('Upload or paste sample data, run the AI queue, export evidence, and prove value without managed writes.')
  || !productHomeReadinessSource.includes('Activate tenant roles, audit, branch data, approved learning, hosted scheduler, and write gates after review.')
  || !productHomeReadinessSource.includes('aria-label="Track launch pack actions"')
  || !productHomeReadinessSource.includes('Retail launch pack')
  || !productHomeReadinessSource.includes('Factory launch pack')
  || !productHomeReadinessSource.includes('Brand launch pack')
  || !productHomeReadinessSource.includes('Branch launch pack')
  ...
  || !productHomeReadinessSource.includes('/settings/#controls')
  || !productHomeReadinessSource.includes('Prepare data')
  || !productHomeReadinessSource.includes('Open workspace')
  || !productHomeReadinessSource.includes('Choose track')
  || !productHomeReadinessSource.includes('aria-label="AI operating tracks"')
  || !productHomeReadinessSource.includes('AI operating tracks')
  || !productHomeReadinessSource.includes('Separate products, one simple operating model.')
  || !productHomeReadinessSource.includes('AI inventory operator')
  || !productHomeReadinessSource.includes('AI MES coordinator')
  || !productHomeReadinessSource.includes('AI site builder')
  || !productHomeReadinessSource.includes('AI order desk')
  || !productHomeReadinessSource.includes('aria-label="AI learning cockpit"')
  || !productHomeReadinessSource.includes('AI learning cockpit')
  || !productHomeReadinessSource.includes('Learn from use, not guesswork.')
  || !productHomeReadinessSource.includes('No customer message, payment, stock move, production write, publish action, or model training runs here.')
  || !productHomeReadinessSource.includes('Export context')
...
if (!appLiveVerifierSource.includes('AI command queue')
  ...
  || !appLiveVerifierSource.includes('SuperMega ranks setup, import, product work, managed activation, and learning handoff into safe next actions.')
  ...
  || !appLiveVerifierSource.includes('Business starter tracks')
  || !appLiveVerifierSource.includes('Pick the business type. SuperMega picks the modules.')
  || !appLiveVerifierSource.includes('Retail or distributor')
  || !appLiveVerifierSource.includes('Factory or workshop')
  || !appLiveVerifierSource.includes('Multi-branch operator')
  || !appLiveVerifierSource.includes('Start with product CSV, stock counts, online order CSV, and payment review proof.')
  || !appLiveVerifierSource.includes('Local enterprise max')
  || !appLiveVerifierSource.includes('Prepared template demo')
  || !appLiveVerifierSource.includes('Minimal controls')
  || !appLiveVerifierSource.includes('Track launch pack actions')
  || !appLiveVerifierSource.includes('Retail launch pack')
  || !appLiveVerifierSource.includes('Factory launch pack')
  || !appLiveVerifierSource.includes('Prepare data')
  || !appLiveVerifierSource.includes('Open workspace')
  || !appLiveVerifierSource.includes('agent_job_chosen')
  || !appLiveVerifierSource.includes('AI operating tracks')
  || !appLiveVerifierSource.includes('Separate products, one simple operating model.')
  || !appLiveVerifierSource.includes('AI MES coordinator')) fail('live_verifier_missing_ai_command_queue_contract')
if (!settingsPageSource.includes('const learningRows = [')
  ...
  || !settingsPageSource.includes("['Data', localRecordCount ? `${localRecordCount} records` : 'Import']")
  ...
  || !settingsPageSource.includes("['Source data', localRecordCount ? `${localRecordCount} prepared` : 'Need records'")
  ...
  || !settingsPageSource.includes('Download request packet')
...
if (!coreSource.includes('const authReady = Boolean(body.authentication?.trusted_gateway_ready || body.authentication?.supabase_user_tokens_ready)')
  ...
  || !settingsPageSource.includes('Export evidence before managed import.')
  || !settingsPageSource.includes('<a className="core-button" download={managedTrialRequestFilename} href={managedTrialRequestHref}>Download request packet</a>')
...
if (!coreSource.includes('const plantRows = [')
  ...
  || !coreSource.includes('data-metric={label.toLowerCase()}')
  || !coreCssSource.includes('.plant-mes-strip span[data-metric="wcm"]')
  || !coreCssSource.includes('.plant-mes-strip span[data-metric="trace"] { display: none; }')
...
if (!websiteSource.includes('aria-label="Website next action"')
  ...
  || !websiteSource.includes("['Package', publishIsCurrent ? 'Ready' : 'Needed']")
...
if (!ecommerceSource.includes("useState<'setup' | 'preview'>('preview')")
  ...
  || !ecommerceCssSource.includes('.ecommerce-heading-actions {\n    width: 100%;\n    display: grid;\n    grid-template-columns: repeat(2, minmax(0, 1fr));')
  || !ecommerceCssSource.includes('.ecommerce-heading-actions .text-link {\n    width: auto;\n    justify-content: center;')
...
if (!websiteSource.includes('starterSetupActive')
  ...
  || !websiteSource.includes('aria-label="Website next action"')
  || !websiteSource.includes('Website operations')
...
if (!productionSource.includes('startProductionMaintenance')
  ...
  || !coreSource.includes('A commissioned asset uses its reviewed owner, procedure, revision, and due date. Completion advances the next due from the strategy interval. No equipment command, telemetry, parts purchase, status change, downtime, or job change occurs.')) fail('production_bounded_maintenance_contract_missing')
