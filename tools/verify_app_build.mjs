...
if (!ecommerceSource.includes('const aiDeskRows = [')
  ...
  || !ecommerceCssSource.includes('.ecommerce-ai-desk-queue')) fail('ecommerce_ai_order_desk_missing')
...
if (!settingsPageSource.includes('const learningRows = [')
  ...
  || !settingsPageSource.includes("['AI', runtime.enterpriseDbReady && runtime.writesReady ? 'Ready' : 'Locked']")
  ...
  || !settingsPageSource.includes('Premium learns only from exported records, accountable actions, and reviewed decisions.')
  || !settingsPageSource.includes('Export AI context package')
  || !settingsPageSource.includes('Premium can rank next actions from reviewed operator behavior after import.')
  ...
  || !settingsPageSource.includes('version: 14')
