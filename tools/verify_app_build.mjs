...
if (!coreSource.includes('const authReady = Boolean(body.authentication?.trusted_gateway_ready || body.authentication?.supabase_user_tokens_ready)')
  ...
  || !coreSource.includes('const activationRows = [')
  || !coreSource.includes("['Auth', runtime.authReady ? 'Ready' : 'Needed']")
  || !coreSource.includes("['Audit', runtime.auditReady ? 'Ready' : 'Needed']")
