...
if (!productHomePageContract.includes('title="Pick a track. Run work."')
...
if (!coreSource.includes('const learningRows = [')
  || !coreSource.includes("['Data', localRecordCount ? `${localRecordCount} records` : 'Import']")
  || !coreSource.includes("['Trust', `${runtime.coverageScore}%`]")
  || !coreSource.includes("['AI', runtime.enterpriseDbReady && runtime.writesReady ? 'Ready' : 'Locked']")
  || !coreSource.includes('aria-label="AI learning readiness"')
  || !coreSource.includes('version: 11')
  || !coreSource.includes('learningRows')) fail('ai_learning_readiness_missing')
...
if (!coreSource.includes('const authReady = Boolean(body.authentication?.trusted_gateway_ready || body.authentication?.supabase_user_tokens_ready)')
  ...
  || !coreSource.includes('runtime.authReady')
  || !coreSource.includes('runtime.auditReady')
  || !coreSource.includes("lazy(() => import('./ManagedActivationRunbook')")
  || !coreSource.includes('<ManagedActivationRunbook runtime={runtime} />')
  || !coreSource.includes('const activationRows: Array<readonly [string, string]> = [')
...
if (!coreSource.includes('className="core-panel next-task-card"')
  ...
  || !coreSource.includes("useState<'workflow' | 'success'>('workflow')")
...
if (!coreSource.includes("lazy(() => import('./ClientDataOnboarding')")
  ...
  || !coreSource.includes('productName={selectedProduct.name}')
  || !coreSource.includes('productSlug={selectedProduct.slug}')
...
const settingsStepNavContract = sourceBlock(coreSource, '      <nav aria-label="Setup steps"', '\n      </nav>')
if (!coreSource.includes('templateId: string')
  ...
  || !coreSource.includes("setupProductFromQuery(setupSearchParams.get('product'))")
  || !coreSource.includes('navigate(clientSetupPath(product), { replace: true })')
  || !coreSource.includes('Your client details were kept.')
  || !coreSource.includes('className="setup-product-grid"')
  || !coreSource.includes('className="setup-template-summary"')
  || !coreSource.includes('<span>Advanced controls</span>')
