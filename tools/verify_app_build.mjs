...
if (!settingsPageSource.includes('const learningRows = [')
  ...
  || !settingsPageSource.includes('version: 17')
...
if (!coreSource.includes('const authReady = Boolean(body.authentication?.trusted_gateway_ready || body.authentication?.supabase_user_tokens_ready)')
  ...
  || !settingsPageSource.includes('<a className="core-button" download={evidenceFilename} href={evidenceHref}>Export evidence</a><a className="core-button primary" href={managedTrialRequestUrl(setup.product, selectedTemplate.id)}>Request managed trial</a>')
