...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceReturnIntentSourceRequestIdBrief } from './ecommerce-return-intent-source-request-id-brief.ts'`,
...
const { projectEcommerceReturnIntentSourceRequestIdBrief } = await import(
...
let intentId = 0
function returnIntent(sourceRequestId = 'REQ-DEFAULT') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceReturnIntentSourceRequestIdBrief(state())
...
{
  const r = projectEcommerceReturnIntentSourceRequestIdBrief(state([returnIntent('REQ-001')]))
...
{
  const r = projectEcommerceReturnIntentSourceRequestIdBrief(state([
    returnIntent('REQ-003'),
    returnIntent('REQ-004'),
  ]))
...
{
  const r = projectEcommerceReturnIntentSourceRequestIdBrief(state([
    returnIntent('REQ-005'),
    returnIntent('REQ-006'),
    returnIntent('REQ-005'),
  ]))
...
{
  const r = projectEcommerceReturnIntentSourceRequestIdBrief(state([
    returnIntent('REQ-007'),
    returnIntent('REQ-008'),
    returnIntent('REQ-009'),
  ]))
...
console.log(`ecommerce-return-intent-source-request-id-brief: ${checks} checks passed`)
