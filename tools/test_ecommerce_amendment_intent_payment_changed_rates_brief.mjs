...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentPaymentChangedRatesBrief } from './ecommerce-amendment-intent-payment-changed-rates-brief.ts'`,
...
const { projectEcommerceAmendmentIntentPaymentChangedRatesBrief } = await import(
...
let intentId = 0
function amendmentIntent({ paymentChanged = false } = {}) {
  intentId++
  return {
...
{
  const r = projectEcommerceAmendmentIntentPaymentChangedRatesBrief(state())
...
{
  const r = projectEcommerceAmendmentIntentPaymentChangedRatesBrief(state([
    amendmentIntent({ paymentChanged: true }),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentPaymentChangedRatesBrief(state([amendmentIntent()]))
...
console.log(`ecommerce-amendment-intent-payment-changed-rates-brief: ${checks} checks passed`)
