...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceCorrectionIntentCustomerMessageSentRatesBrief } from './ecommerce-correction-intent-customer-message-sent-rates-brief.ts'`,
...
const { projectEcommerceCorrectionIntentCustomerMessageSentRatesBrief } = await import(
...
let intentId = 0
function correctionIntent({ customerMessageSent = false } = {}) {
  intentId++
  return {
...
{
  const r = projectEcommerceCorrectionIntentCustomerMessageSentRatesBrief(state())
...
{
  const r = projectEcommerceCorrectionIntentCustomerMessageSentRatesBrief(state([correctionIntent()]))
...
console.log(`ecommerce-correction-intent-customer-message-sent-rates-brief: ${checks} checks passed`)
