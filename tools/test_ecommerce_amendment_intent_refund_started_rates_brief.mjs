...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentRefundStartedRatesBrief } from './ecommerce-amendment-intent-refund-started-rates-brief.ts'`,
...
const { projectEcommerceAmendmentIntentRefundStartedRatesBrief } = await import(
...
let intentId = 0
function amendmentIntent({ refundStarted = false } = {}) {
  intentId++
  return {
...
{
  const r = projectEcommerceAmendmentIntentRefundStartedRatesBrief(state())
...
{
  const r = projectEcommerceAmendmentIntentRefundStartedRatesBrief(state([
    amendmentIntent({ refundStarted: true }),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentRefundStartedRatesBrief(state([amendmentIntent()]))
...
console.log(`ecommerce-amendment-intent-refund-started-rates-brief: ${checks} checks passed`)
