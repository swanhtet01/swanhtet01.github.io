#!/usr/bin/env node
import { reviewIdentity } from './prepare_exact_head_codex_review_trigger.mjs'
...
function sameState(plan, fresh) { return JSON.stringify(fresh.pullRequest) === JSON.stringify(plan.pullRequest) && fresh.pullRequest.number === plan.pullRequest.number && fresh.pullRequest.base === plan.pullRequest.base && fresh.pullRequest.head === plan.pullRequest.head && fresh.pullRequest.headUpdatedAt === plan.pullRequest.headUpdatedAt && JSON.stringify(fresh.observed.exactHeadChecks) === JSON.stringify(plan.observed.exactHeadChecks) && JSON.stringify(fresh.observed.codexReviewComments) === JSON.stringify(plan.observed.codexReviewComments) && JSON.stringify(fresh.observed.headTimeline) === JSON.stringify(plan.observed.headTimeline) && fresh.observed.exactHeadReviews.length === 0 && fresh.observed.reviewNamedChecks.length === 0 && fresh.observed.currentHeadTriggerCount === 0 }
...
export async function applyExactHeadCodexReviewTrigger({ plan, planPayload = null, planFileDigest = null, fetchJson = fetchGitHubJson, request = fetch, confirmer = confirmExactHeadCodexReviewOwnerClick, env = process.env, githubCli = githubCliDefault, gitState = localGitState(), now = () => new Date(), toolDigests = sourceToolDigests(), nonce = () => randomBytes(32).toString('hex') } = {}) {
  ...
  try {
    ...
    const identity = reviewIdentity(after)
    if (['number', 'headRef', 'baseRef', 'headRepository', 'baseRepository', 'head', 'base'].some((key) => identity[key] !== plan.pullRequest[key])) fail('exact_head_codex_review_trigger_post_write_head_drift')
