...
function sameState(plan, fresh) { return fresh.pullRequest.number === plan.pullRequest.number && fresh.pullRequest.base === plan.pullRequest.base && fresh.pullRequest.head === plan.pullRequest.head && fresh.pullRequest.headUpdatedAt === plan.pullRequest.headUpdatedAt && JSON.stringify(fresh.observed.exactHeadChecks) === JSON.stringify(plan.observed.exactHeadChecks) && JSON.stringify(fresh.observed.codexReviewComments) === JSON.stringify(plan.observed.codexReviewComments) && JSON.stringify(fresh.observed.headTimeline) === JSON.stringify(plan.observed.headTimeline) && fresh.observed.exactHeadReviews.length === 0 && fresh.observed.reviewNamedChecks.length === 0 && fresh.observed.currentHeadTriggerCount === 0 }
