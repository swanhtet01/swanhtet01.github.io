...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentCustomerMessageFromFulfilmentBrief } from './ecommerce-amendment-intent-customer-message-from-fulfilment-brief.ts'`,
...
const { projectEcommerceAmendmentIntentCustomerMessageFromFulfilmentBrief } = await import(
...
let intentId = 0
function amendmentIntent(customerMessageSent = false, fromFulfilment = 'delivery') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceAmendmentIntentCustomerMessageFromFulfilmentBrief(state())
  check(r.totalIntents === 0, 'empty: totalIntents 0')
  check(r.messageSentCount === 0, 'empty: messageSentCount 0')
  check(r.noMessageCount === 0, 'empty: noMessageCount 0')
}

// 2. Message sent + from delivery
{
  const r = projectEcommerceAmendmentIntentCustomerMessageFromFulfilmentBrief(state([
...
console.log(`ecommerce-amendment-intent-customer-message-from-fulfilment-brief: ${checks} checks passed`)
