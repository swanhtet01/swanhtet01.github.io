...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief } from './ecommerce-reschedule-intent-fulfilment-order-status-brief.ts'`,
...
const { projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief } = await import(
...
let intentId = 0
function rescheduleIntent(fulfilment = 'delivery', orderStatus = 'confirmed') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief(state())
...
{
  const r = projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief(state([
    rescheduleIntent('delivery', 'confirmed'),
  ]))
...
{
  const r = projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief(state([
    rescheduleIntent('pickup', 'preparing'),
  ]))
...
{
  const r = projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief(state([
    rescheduleIntent('delivery', 'ready'),
  ]))
...
{
  const r = projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief(state([
    rescheduleIntent('pickup', 'confirmed'),
  ]))
...
// 6. All 6 cells
{
  const r = projectEcommerceRescheduleIntentFulfilmentOrderStatusBrief(state([
    rescheduleIntent('delivery', 'confirmed'),
    rescheduleIntent('delivery', 'preparing'),
    rescheduleIntent('delivery', 'ready'),
    rescheduleIntent('pickup', 'confirmed'),
    rescheduleIntent('pickup', 'preparing'),
    rescheduleIntent('pickup', 'ready'),
  ]))
...
console.log(`ecommerce-reschedule-intent-fulfilment-order-status-brief: ${checks} checks passed`)
