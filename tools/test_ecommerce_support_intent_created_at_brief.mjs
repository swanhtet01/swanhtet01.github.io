...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceSupportIntentCreatedAtBrief } from './ecommerce-support-intent-created-at-brief.ts'`,
...
const { projectEcommerceSupportIntentCreatedAtBrief } = await import(
...
let intentId = 0
function supportIntent(createdAt = '2026-08-01T09:00:00Z') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceSupportIntentCreatedAtBrief(state())
...
{
  const r = projectEcommerceSupportIntentCreatedAtBrief(state([
    supportIntent('2026-08-01T09:00:00Z'),
  ]))
...
{
  const r = projectEcommerceSupportIntentCreatedAtBrief(state([
    supportIntent('2026-08-05T08:00:00Z'),
    supportIntent('2026-08-05T16:00:00Z'),
  ]))
...
{
  const r = projectEcommerceSupportIntentCreatedAtBrief(state([
    supportIntent('2026-07-25T00:00:00Z'),
    supportIntent('2026-08-01T00:00:00Z'),
  ]))
...
{
  const r = projectEcommerceSupportIntentCreatedAtBrief(state([
    supportIntent('2026-08-10T08:00:00Z'),
    supportIntent('2026-08-02T12:00:00Z'),
    supportIntent('2026-08-06T15:00:00Z'),
  ]))
...
{
  const r = projectEcommerceSupportIntentCreatedAtBrief(state([
    supportIntent('2026-08-08T00:00:00Z'),
    supportIntent('2026-08-08T00:00:00Z'),
  ]))
...
console.log(`ecommerce-support-intent-created-at-brief: ${checks} checks passed`)
