...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceCancellationIntentOrderIdBrief } from './ecommerce-cancellation-intent-order-id-brief.ts'`,
...
const { projectEcommerceCancellationIntentOrderIdBrief } = await import(
...
let intentId = 0
function cancellationIntent(orderId = 'ORD-DEFAULT') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceCancellationIntentOrderIdBrief(state())
...
{
  const r = projectEcommerceCancellationIntentOrderIdBrief(state([cancellationIntent('ORD-001')]))
...
{
  const r = projectEcommerceCancellationIntentOrderIdBrief(state([
    cancellationIntent('ORD-003'),
    cancellationIntent('ORD-004'),
  ]))
...
{
  const r = projectEcommerceCancellationIntentOrderIdBrief(state([
    cancellationIntent('ORD-005'),
    cancellationIntent('ORD-006'),
    cancellationIntent('ORD-005'),
  ]))
...
{
  const r = projectEcommerceCancellationIntentOrderIdBrief(state([
    cancellationIntent('ORD-007'),
    cancellationIntent('ORD-008'),
    cancellationIntent('ORD-009'),
  ]))
...
console.log(`ecommerce-cancellation-intent-order-id-brief: ${checks} checks passed`)
