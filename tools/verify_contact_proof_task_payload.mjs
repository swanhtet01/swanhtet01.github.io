...
assert.ok(proofTask.source_pack_request)
assert.equal(proofTask.source_pack_request.status, 'source_pack_request_ready')
assert.equal(proofTask.source_pack_request.request_type, 'client_source_pack_intake')
assert.equal(proofTask.source_pack_request.lead_id, record.lead_id)
assert.equal(proofTask.source_pack_request.action_id, record.task_id)
assert.equal(proofTask.source_pack_request.intake_url, 'https://supermega.dev/app/source-pack?lead=LEAD-TEST123&action=TASK-TEST123')
assert.equal(proofTask.source_pack_request.external_action_state, 'blocked_until_owner_approval')
assert.equal(proofTask.source_pack_request.payment_request_state, 'blocked_until_owner_approval')
assert.equal(proofTask.source_pack_request.real_mrr_delta, 0)
assert.ok(proofTask.source_pack_request.minimum_sources.length >= 3)
assert.ok(proofTask.source_pack_request.client_message.includes('Source-pack intake link: https://supermega.dev/app/source-pack?lead=LEAD-TEST123&action=TASK-TEST123'))
assert.ok(proofTask.source_pack_request_json.includes('"request_type": "client_source_pack_intake"'))
assert.ok(proofTask.source_pack_request_packet.includes('Minimum source pack'))
assert.ok(proofTask.owner_onboarding_alert)
assert.equal(proofTask.owner_onboarding_alert.status, 'owner_alert_ready')
assert.equal(proofTask.owner_onboarding_alert.alert_type, 'ai_workcell_source_pack_room')
assert.equal(proofTask.owner_onboarding_alert.external_action_state, 'draft_only_no_send')
assert.equal(proofTask.owner_onboarding_alert.real_mrr_delta, 0)
assert.ok(proofTask.owner_onboarding_alert.message.includes('Source pack room: https://supermega.dev/app/source-pack?lead=LEAD-TEST123&action=TASK-TEST123'))
...
assert.equal(action.payload.source_pack_request.status, 'source_pack_request_ready')
assert.equal(action.payload.source_pack_request.intake_url, proofTask.source_pack_request.intake_url)
assert.equal(action.payload.source_pack_request_packet, proofTask.source_pack_request_packet)
assert.equal(action.payload.owner_onboarding_alert.alert_type, 'ai_workcell_source_pack_room')
assert.ok(action.payload.owner_onboarding_alert.message.includes('Owner action: send source request only after approval.'))
...
assert.ok(alert.includes('Source pack room: https://supermega.dev/app/source-pack?lead=LEAD-TEST123&action=TASK-TEST123'))
