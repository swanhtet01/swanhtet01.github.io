[CmdletBinding()]
param(
    [ValidateSet('Install', 'Status', 'Remove', 'Repair', 'Migrate')]
    [string]$Mode = 'Status',
    [switch]$SelfTest
)
...
function Test-CeoTaskPolicyFor([object]$Task, [string]$ExpectedInterval, [string]$ExpectedIdleWait) {
    ...
    return (
        ...
        [string]::Equals([string]$trigger.Repetition.Interval, $ExpectedInterval, [StringComparison]::Ordinal) -and
        ...
        $Task.Settings.IdleSettings.WaitTimeout -eq $ExpectedIdleWait -and
...
function Test-CeoTaskPolicy([object]$Task) {
    return Test-CeoTaskPolicyFor $Task $Interval 'PT4H'
}

function Test-CeoTaskLegacySixHourPolicy([object]$Task) {
    return Test-CeoTaskPolicyFor $Task 'PT6H' 'PT6H'
}
...
function Test-CeoTaskGuardShape([object]$Task) {
...
function Test-CeoTaskRepairable([object]$Task) {
    return (Test-CeoTaskPolicy $Task) -and (Test-CeoTaskGuardShape $Task)
}

function Test-CeoTaskMigratable([object]$Task) {
    return (Test-CeoTaskLegacySixHourPolicy $Task) -and (Test-CeoTaskGuardShape $Task)
}
...
try {
    ...
    if ($Mode -eq 'Migrate' -and $null -ne $existing) {
        if (Test-CeoTask $existing) {
            Write-CeoTaskReceipt -Status 'ready' -Task $existing -Changed $false
            exit 0
        }
        if (-not (Test-CeoTaskMigratable $existing)) { throw 'ally_ceo_task_migrate_refused_untrusted_definition' }
        Unregister-ScheduledTask -TaskName $TaskName -Confirm:$false -ErrorAction Stop
        $MutationCommitted = $true
        $existing = $null
    } elseif ($Mode -eq 'Migrate') {
        throw 'ally_ceo_task_migrate_missing_definition'
    }
    ...
    $settings = New-ScheduledTaskSettingsSet -StartWhenAvailable -AllowStartIfOnBatteries -DontStopIfGoingOnBatteries -RunOnlyIfIdle -IdleDuration (New-TimeSpan -Minutes 10) -IdleWaitTimeout (New-TimeSpan -Hours 4) -DontStopOnIdleEnd -ExecutionTimeLimit (New-TimeSpan -Hours 2) -MultipleInstances IgnoreNew
