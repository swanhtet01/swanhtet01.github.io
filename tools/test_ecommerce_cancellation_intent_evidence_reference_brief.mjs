...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceCancellationIntentEvidenceReferenceBrief } from './ecommerce-cancellation-intent-evidence-reference-brief.ts'`,
...
const { projectEcommerceCancellationIntentEvidenceReferenceBrief } = await import(
...
let intentId = 0
function cancellationIntent(evidenceReference = 'ev-default') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceCancellationIntentEvidenceReferenceBrief(state())
...
{
  const r = projectEcommerceCancellationIntentEvidenceReferenceBrief(state([cancellationIntent('ECOMMERCE-CANCELLATION:A')]))
...
{
  const r = projectEcommerceCancellationIntentEvidenceReferenceBrief(state([
    cancellationIntent('ECOMMERCE-CANCELLATION:B'),
    cancellationIntent('ECOMMERCE-CANCELLATION:B'),
  ]))
...
{
  const r = projectEcommerceCancellationIntentEvidenceReferenceBrief(state([
    cancellationIntent('ECOMMERCE-CANCELLATION:C'),
    cancellationIntent('ECOMMERCE-CANCELLATION:D'),
  ]))
...
{
  const r = projectEcommerceCancellationIntentEvidenceReferenceBrief(state([
    cancellationIntent('ECOMMERCE-CANCELLATION:E'),
    cancellationIntent('ECOMMERCE-CANCELLATION:F'),
    cancellationIntent('ECOMMERCE-CANCELLATION:E'),
  ]))
...
{
  const r = projectEcommerceCancellationIntentEvidenceReferenceBrief(state([
    cancellationIntent('ECOMMERCE-CANCELLATION:G'),
    cancellationIntent('ECOMMERCE-CANCELLATION:H'),
    cancellationIntent('ECOMMERCE-CANCELLATION:I'),
  ]))
...
console.log(`ecommerce-cancellation-intent-evidence-reference-brief: ${checks} checks passed`)
