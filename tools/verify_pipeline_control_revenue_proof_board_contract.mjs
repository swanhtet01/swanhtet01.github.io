...
assert.equal(board.next_cash_actions.length, 2)
assert.ok(board.next_cash_actions.some((action) => action.next_action === 'bank_reconcile_payment_proof' && action.revenue_claim_state === 'proof_backed_not_bank_verified'))
assert.ok(board.next_cash_actions.some((action) => action.next_action === 'collect_retainer_payment_proof' && action.revenue_claim_state === 'mrr_still_zero_until_payment_proof'))
assert.ok(board.payment_ledger_csv.includes('KBZ-PROOF-001'))
assert.ok(board.payment_ledger_csv.includes('"1200000"'))
assert.ok(board.next_cash_actions_csv.includes('bank_reconcile_payment_proof'))
