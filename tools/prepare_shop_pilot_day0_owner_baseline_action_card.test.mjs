...
test('renders an owner-baseline action card without local paths, identity, or authority', () => {
  ...
  assert.equal(card.action.completionSignal, 'owner_safe_baseline_completion_receipt_digest')
  assert.equal(card.minimumEvidence.baselinePacketVerificationRequired, true)
  assert.equal(card.commandPlan.atomicBaselineCompletion, true)
  assert.equal(card.commandPlan.lintPerformedInsideAtomicCommand, true)
  assert.equal(card.commandPlan.writesNothingOnValidationFailure, true)
  assert.equal(card.commandPlan.completionReceiptRequired, true)
  assert.ok(card.commandPlan.commands.some((command) => command.includes('shop:pilot:baseline-complete')
    && command.includes('--input "<private-baseline-input.json>"')
    && command.includes('--output-dir "<private-baseline-completion-directory>"')))
  assert.equal(card.commandPlan.commands.some((command) => command.includes('--lint-input')), false)
  assert.ok(card.commandPlan.commands.some((command) => command.includes('<private-baseline-completion-directory>/owner-safe-baseline-packet.json')))
  ...
  assert.match(markdown, /requires `baseline_input_ready`/)
  ...
  assert.match(markdown, /shop:pilot:baseline-complete/)
  assert.match(markdown, /writes nothing if validation fails/i)
  assert.match(markdown, /sealed completion receipt/i)
