...
const homeProductPreviewScript = `<script>(function(){
var root=document.querySelector('[data-product-preview]');if(!root)return;
var image=root.querySelector('[data-product-preview-image]'),mobileSource=root.querySelector('[data-product-preview-mobile-source]'),label=root.querySelector('[data-product-preview-label]'),description=root.querySelector('[data-product-preview-description]'),openLink=root.querySelector('[data-preview-open]'),proofLink=root.querySelector('[data-preview-proof-link]'),buttons=Array.from(root.querySelectorAll('[data-product-preview-button]'));
if(!image||!label||!description||!openLink||!buttons.length)return;
var views={shop:{src:'/live-shop-workspace.png',mobileSrc:'/live-shop-mobile.png',alt:'Current Shop workspace showing priority checks, sales, cash, customers, and stock',label:'Shop / live workspace',description:'Sales, stock, customers, and daily close in one operating flow.',href:'${shopStartUrl}',cta:'Open Shop demo',proofLabel:'Open the live Shop workspace'},plant:{src:'/live-plant-workspace.png',mobileSrc:'/live-plant-mobile.png',alt:'Current Plant workspace showing production plan navigation and machine state across two lines',label:'Plant / live workspace',description:'Production plans, machine state, and handoffs in one floor view.',href:'${plantStartUrl}',cta:'Open Plant demo',proofLabel:'Open the live Plant workspace'}};
function show(id){var view=views[id];if(!view)return;buttons.forEach(function(button){button.setAttribute('aria-pressed',String(button.getAttribute('data-product-preview-button')===id));});label.textContent=view.label;description.textContent=view.description;openLink.href=view.href;openLink.textContent=view.cta;openLink.setAttribute('aria-label',view.cta);if(proofLink){proofLink.href=view.href;proofLink.setAttribute('aria-label',view.proofLabel);}if(mobileSource)mobileSource.srcset=view.mobileSrc;if(image.getAttribute('data-view')===id)return;image.classList.add('is-switching');image.alt=view.alt;image.setAttribute('data-view',id);image.src=view.src;window.setTimeout(function(){image.classList.remove('is-switching');},360);}
image.addEventListener('load',function(){image.classList.remove('is-switching');});buttons.forEach(function(button){button.addEventListener('click',function(){show(button.getAttribute('data-product-preview-button'));});});var preloadDesktop=new Image(),preloadMobile=new Image();preloadDesktop.src=views.plant.src;preloadMobile.src=views.plant.mobileSrc;
})();</script>`
...
const homeHtml = documentHtml({
  ...
  style: `
    ...
    @media (max-width: 720px) {
      ...
      .preview-media { height: clamp(232px, 67vw, 272px); }
      .preview-media picture, .preview-media img { height: 100%; }
      .preview-media img { width: 100%; aspect-ratio: auto; object-fit: cover; object-position: top; }
    ...
    <div class="hero-inner" data-product-preview>
      <div class="hero-stage">
        ...
        <div class="hero-preview">
          ...
          <div class="preview-media"><picture><source media="(max-width: 720px)" data-product-preview-mobile-source srcset="/live-shop-mobile.png" /><img data-product-preview-image data-view="shop" src="/live-shop-workspace.png" alt="Current Shop workspace showing priority checks, sales, cash, customers, and stock" width="1600" height="480" fetchpriority="high" /></picture></div>
