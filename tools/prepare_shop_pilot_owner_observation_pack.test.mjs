...
test('builds a privacy-safe owner observation pack bound to current release control', () => {
  ...
  assert.equal(pack.ownerAction.completionSignal, 'owner_safe_baseline_completion_receipt_digest')
  ...
  assert.equal(pack.commandPlan.atomicBaselineCompletion, true)
  assert.equal(pack.commandPlan.lintPerformedInsideAtomicCommand, true)
  assert.equal(pack.commandPlan.writesNothingOnValidationFailure, true)
  assert.equal(pack.commandPlan.completionReceiptRequired, true)
  assert.ok(pack.commandPlan.commands.some((command) => command.includes('shop:pilot:baseline-complete')
    && command.includes('--input "<private-baseline-input.json>"')
    && command.includes('--output-dir "<private-baseline-completion-directory>"')))
  assert.ok(
    pack.commandPlan.commands.findIndex((command) => command.includes('shop:pilot:baseline-complete'))
      < pack.commandPlan.commands.findIndex((command) => command.includes('shop:pilot:launch-gate')),
  )
  ...
  assert.match(markdown, /Required completion signal: `owner_safe_baseline_completion_receipt_digest`/)
  assert.match(markdown, /shop:pilot:baseline-complete/)
  assert.match(markdown, /writes nothing when validation fails/i)
