...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentEvidenceReferenceBrief } from './ecommerce-amendment-intent-evidence-reference-brief.ts'`,
...
const { projectEcommerceAmendmentIntentEvidenceReferenceBrief } = await import(
...
let intentId = 0
function amendmentIntent(evidenceReference = 'ev-default') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceAmendmentIntentEvidenceReferenceBrief(state())
...
{
  const r = projectEcommerceAmendmentIntentEvidenceReferenceBrief(state([amendmentIntent('ECOMMERCE-AMENDMENT:A')]))
  check(r.totalIntents === 1, 'single: totalIntents 1')
  check(r.uniqueReferences === 1, 'single: uniqueReferences 1')
  check(r.topReference === 'ECOMMERCE-AMENDMENT:A', 'single: topReference')
...
{
  const r = projectEcommerceAmendmentIntentEvidenceReferenceBrief(state([
    amendmentIntent('ECOMMERCE-AMENDMENT:B'),
    amendmentIntent('ECOMMERCE-AMENDMENT:B'),
  ]))
  check(r.totalIntents === 2, 'same-ref: totalIntents 2')
  check(r.uniqueReferences === 1, 'same-ref: uniqueReferences 1')
  check(r.topReference === 'ECOMMERCE-AMENDMENT:B', 'same-ref: topReference')
...
{
  const r = projectEcommerceAmendmentIntentEvidenceReferenceBrief(state([
    amendmentIntent('ECOMMERCE-AMENDMENT:C'),
    amendmentIntent('ECOMMERCE-AMENDMENT:D'),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentEvidenceReferenceBrief(state([
    amendmentIntent('ECOMMERCE-AMENDMENT:E'),
    amendmentIntent('ECOMMERCE-AMENDMENT:F'),
    amendmentIntent('ECOMMERCE-AMENDMENT:E'),
  ]))
  check(r.totalIntents === 3, 'dominant: totalIntents 3')
  check(r.uniqueReferences === 2, 'dominant: uniqueReferences 2')
  check(r.topReference === 'ECOMMERCE-AMENDMENT:E', 'dominant: topReference')
...
{
  const r = projectEcommerceAmendmentIntentEvidenceReferenceBrief(state([
    amendmentIntent('ECOMMERCE-AMENDMENT:G'),
    amendmentIntent('ECOMMERCE-AMENDMENT:H'),
    amendmentIntent('ECOMMERCE-AMENDMENT:I'),
  ]))
...
console.log(`ecommerce-amendment-intent-evidence-reference-brief: ${checks} checks passed`)
