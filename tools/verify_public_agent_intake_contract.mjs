...
assert.equal(shopWorkspaceTask.template_id, 'deskpos-quickstart')
assert.equal(shopWorkspaceTask.product_area, 'Shop')
assert.equal(shopWorkspaceTask.solution_route.delivery_lane, 'pos_launch_workcell')
assert.match(shopWorkspaceTask.operator_brief, /Shop private workspace first proof/i)
assert.ok(shopWorkspaceTask.implementation_blueprint_pack.modules.includes('checkout_or_booking'))
...
assert.equal(plantWorkspaceTask.template_id, 'factory-ops-ledger')
assert.equal(plantWorkspaceTask.product_area, 'Plant')
assert.equal(plantWorkspaceTask.solution_route.delivery_lane, 'factory_ops_workcell')
