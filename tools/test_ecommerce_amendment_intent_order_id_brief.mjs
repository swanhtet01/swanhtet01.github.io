...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentOrderIdBrief } from './ecommerce-amendment-intent-order-id-brief.ts'`,
...
const { projectEcommerceAmendmentIntentOrderIdBrief } = await import(
...
let intentId = 0
function amendmentIntent(orderId = 'ORD-DEFAULT') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceAmendmentIntentOrderIdBrief(state())
...
{
  const r = projectEcommerceAmendmentIntentOrderIdBrief(state([amendmentIntent('ORD-001')]))
...
{
  const r = projectEcommerceAmendmentIntentOrderIdBrief(state([
    amendmentIntent('ORD-003'),
    amendmentIntent('ORD-004'),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentOrderIdBrief(state([
    amendmentIntent('ORD-005'),
    amendmentIntent('ORD-006'),
    amendmentIntent('ORD-005'),
  ]))
...
{
  const r = projectEcommerceAmendmentIntentOrderIdBrief(state([
    amendmentIntent('ORD-007'),
    amendmentIntent('ORD-008'),
    amendmentIntent('ORD-009'),
  ]))
...
console.log(`ecommerce-amendment-intent-order-id-brief: ${checks} checks passed`)
