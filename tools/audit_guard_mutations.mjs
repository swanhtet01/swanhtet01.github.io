...
const MUTATIONS = [
  ...
  ['test_shop_inventory_setup_reachable.mjs', COMMERCE,
  ...
  ['test_commerce_write_fails_closed.mjs', COMMERCE,
    "        if (storage.getItem(COMMERCE_KEY) !== serialized) return { ok: false, error: 'Commerce storage did not confirm the write.' } as const",
    '        void serialized',
    'a silently dropped write is reported to the operator as a completed sale'],
