...
requireContract('one bounded agent operating model is authoritative',
  ...
  && portfolio.completedLocalAutomations.length === 21
  ...
  && portfolio.localImprovementQueue.find((entry) => entry.status === 'ready-local')?.workOrderId === 'ecommerce-cart-line-remove-recovery'
...
requireContract('local product improvement stays separate from managed-pilot authority',
  ...
  && portfolio.localImprovementQueue.length === 1
  && portfolio.localImprovementQueue[0].workOrderId === 'ecommerce-cart-line-remove-recovery')
