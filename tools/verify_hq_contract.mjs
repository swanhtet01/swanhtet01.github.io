...
requireContract('agent roster consolidation is recorded',
  ...
  && workboard.includes('Current accepted agent-operations checkpoint: `f642962`')
  && now.includes('agent operations `f642962`')
  && now.includes('operations `63a245f`')
  && now.includes('unchanged evidence-bound missions reuse for 24 hours')
  ...
  && now.includes('A provider-neutral Storage verifier now proves explicit anonymous-list denial')
  && now.includes('its 11-case self-test makes zero network requests')
...
requireContract('agent security brief is reconciled to current controls',
  agentSecurity.includes('Agent-operations checkpoint: `f642962`')
  ...
  && agentSecurity.includes('Prepare one redacted, owner-ready isolated-target audit handoff')
...
requireContract('accepted core checkpoints lead directly to real work',
  workboard.includes('Current accepted product checkpoint: `52917c5`')
  ...
  && now.includes('Current local checkpoints: product `52917c5`')
  && now.includes('Core first-action QA leads Shop Stock')
  && now.includes('The active delivery focus is:')
  && now.includes('Plant Jobs now has a lazy order-execution layer')
  && now.includes('one manifest-backed smart import')
  && now.includes('Product checkpoint `52917c5` is on a clean fast-forward descendant')
