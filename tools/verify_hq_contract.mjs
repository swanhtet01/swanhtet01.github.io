...
requireContract('accepted core checkpoints lead directly to real work',
  workboard.includes('Current accepted product checkpoint: `3cd4825`')
  ...
  && now.includes('Current local checkpoints: product `3cd4825`')
  ...
  && now.includes('PR #264 merged the trustworthy Ecommerce checkout and payment/fulfilment contract')
...
requireContract('release history is retained and current live state is discoverable',
  ...
  && now.includes('Both `supermega.dev` and `app.supermega.dev` serve exact commit `f00b1e3e`'))
requireContract('workboard release authority and active execution order are current',
  ...
  && workboard.includes('Current accepted release checkpoint: `f00b1e3ee163714e7c6f1db4a7f17c3cac73de46`')
  && workboardExecutionOrder.includes('paired live release `f00b1e3e`')
