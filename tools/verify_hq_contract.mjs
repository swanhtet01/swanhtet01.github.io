...
requireContract('Ally CEO execution uses one exact local-company run and no external action surface',
  ...
  && allyCeoLocalCycleText.indexOf('host: validateReadOnlyReplayHost(audit, memoryRecovery)') < allyCeoLocalCycleText.indexOf('if (execute && recoverMemory && canAttemptMemoryRecovery(audit))')
  ...
  && allyCeoLocalCycleText.includes('if (scheduled && !dependencies.runCommand)')
  && allyCeoLocalCycleText.indexOf('if (scheduled && !dependencies.runCommand)') < allyCeoLocalCycleText.indexOf("let audit = parseJson(await runCommand({ kind: 'audit'")
