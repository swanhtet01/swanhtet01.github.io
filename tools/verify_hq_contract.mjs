...
requireContract('accepted core checkpoints lead directly to real work',
  workboard.includes('Current accepted product checkpoint: `d941cad`')
  ...
  && workboard.includes('Keep product checkpoint `d941cad`')
  && workboard.includes('current 208-commit local product delta')
  && now.includes('Checkpoint `d941cad` is the accepted local product')
  && now.includes('Core first-action QA leads Shop Stock')
  && now.includes('Setup, Save, receipt, and handoff focus the next required step')
  && now.includes('Review labels source, customer, SKU/price, payment, full promise, fulfilment, order ID, and stock delta')
  && now.includes('React Router is isolated in a 43,870-byte cacheable chunk')
  && now.includes('largest JavaScript chunk is 463,534 bytes')
  && now.includes('fast-forward 162 commits beyond remote PR #258 head `338b6fd`')
