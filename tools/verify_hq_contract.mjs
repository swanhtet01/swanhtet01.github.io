...
requireContract('one bounded agent operating model is authoritative',
  ...
  && portfolioSource.completedLocalAutomations.length === 40
  && Array.isArray(portfolio.completedLocalAutomations)
  && portfolio.completedLocalAutomations.length === 112
  ...
  && portfolio.localImprovementQueue.find((entry) => entry.status === 'ready-local')?.workOrderId === 'shop-spa-payment-and-daily-close-proof'
...
requireContract('local product improvement stays separate from managed-pilot authority',
  ...
  && portfolio.localImprovementQueue[0].workOrderId === 'shop-spa-payment-and-daily-close-proof')
