...
requireContract('accepted core checkpoints lead directly to real work',
  workboard.includes('Current accepted product checkpoint: `cb4d715`')
  ...
  && workboard.includes('Keep product checkpoint `cb4d715`')
  && workboard.includes('current 206-commit local product delta')
  && now.includes('Checkpoint `cb4d715` is the accepted local product')
  && now.includes('Core first-action QA leads Shop Stock')
  && now.includes('Unsaved Preview has one `Finish setup`')
  && now.includes('Shop handoff focuses Payment')
  && now.includes('React Router is isolated in a 43,870-byte cacheable chunk')
  && now.includes('largest JavaScript chunk is 463,354 bytes')
  && now.includes('fast-forward 160 commits beyond remote PR #258 head `338b6fd`')
