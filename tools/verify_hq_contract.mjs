...
requireContract('one bounded agent operating model is authoritative',
  ...
  && portfolioSource.completedLocalAutomations.length === 41
  && Array.isArray(portfolio.completedLocalAutomations)
  && portfolio.completedLocalAutomations.length === 113
  ...
  && portfolio.localImprovementQueue.find((entry) => entry.status === 'ready-local')?.workOrderId === 'shop-spa-role-access-and-staff-handoff'
...
requireContract('local product improvement stays separate from managed-pilot authority',
  ...
  && portfolio.localImprovementQueue[0].workOrderId === 'shop-spa-role-access-and-staff-handoff')
