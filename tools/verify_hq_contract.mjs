...
requireContract('accepted core checkpoints lead directly to real work',
  workboard.includes('Current accepted product checkpoint: `b855523`')
  ...
  && workboard.includes('Keep product checkpoint `b855523`')
  && workboard.includes('current 204-commit local product delta')
  && now.includes('Checkpoint `b855523` is the accepted local product')
  && now.includes('Core first-action QA leads Shop Stock')
  && now.includes('Unsaved Preview has one `Finish setup` handoff')
  && now.includes('React Router is isolated in a 43,870-byte cacheable chunk')
  && now.includes('largest JavaScript chunk is 463,329 bytes')
  && now.includes('fast-forward 158 commits beyond remote PR #258 head `338b6fd`')
