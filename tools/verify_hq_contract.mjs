...
requireContract('accepted core checkpoints lead directly to real work',
  workboard.includes('Current accepted product checkpoint: `a625b4a`')
  ...
  && now.includes('Current local checkpoints: product `a625b4a`')
  && now.includes('Core first-action QA leads Shop Stock')
  && now.includes('The active delivery focus is:')
  && now.includes('Plant Jobs persists a managed 12-material BOM/routing chain')
  && now.includes('multi-operation WIP, actual minutes')
  && now.includes('one manifest-backed smart import')
  && now.includes('Product checkpoint `a625b4a` is on a clean fast-forward descendant')
