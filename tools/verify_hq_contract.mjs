...
requireContract('one bounded agent operating model is authoritative',
  ...
  && portfolio.completedLocalAutomations.length === 20
  ...
  && portfolio.localImprovementQueue.find((entry) => entry.status === 'ready-local')?.workOrderId === 'website-page-readiness-recovery'
...
requireContract('local product improvement stays separate from managed-pilot authority',
  ...
  && portfolio.localImprovementQueue.length === 1
  && portfolio.localImprovementQueue[0].workOrderId === 'website-page-readiness-recovery')
