...
test('reports missing Day-0 prerequisites without allowing external effects', () => {
  ...
  assert.deepEqual(packet.ownerPrivateBaselineChecklist.requiredFlows.map((flow) => flow.id), ['manual_order', 'package_redemption'])
  ...
  assert.ok(packet.ownerPrivateBaselineChecklist.stopConditions.includes('raw_identity_or_private_note_would_enter_public_packet'))
...
test('renders Markdown and verifies CLI packet without private values', async () => {
  ...
  try {
    ...
    assert.match(markdown, /raw_identity_or_private_note_would_enter_public_packet/)
