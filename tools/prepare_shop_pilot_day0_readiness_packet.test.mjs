...
test('reports missing Day-0 prerequisites without allowing external effects', () => {
  ...
  assert.ok(packet.privateCommands.some((command) => command.includes('--lint-input "<private-baseline-input.json>"') && command.includes('--output "<owner-safe-baseline-preflight.json>"')))
  assert.ok(packet.privateCommands.some((command) => command.includes('--verify-preflight "<owner-safe-baseline-preflight.json>"')))
  assert.ok(packet.privateCommands.some((command) => command.includes('--output "<owner-safe-launch-gate-report.json>"')))
  assert.ok(packet.privateCommands.some((command) => command.includes('--verify "<owner-safe-baseline-packet.json>"')))
...
test('accepts intake-only state while requiring owner-observed baseline', () => {
  ...
  assert.equal(packet.nextOwnerPrivateStep.baselinePacketVerificationRequired, true)
  assert.equal(packet.nextOwnerPrivateStep.completionSignal, 'public_safe_baseline_packet_digest')
  assert.deepEqual(packet.nextOwnerPrivateStep.requiredPrivateInputs, ['manual_order_runs', 'package_redemption_runs', 'daily_close_runs', 'daily_close_calendar_dates', 'daily_close_minutes', 'exception_count'])
  assert.match(packet.ownerAction, /three daily-close runs across three distinct close dates, then generate and verify/)
  assert.equal(packet.ownerPrivateObservationBridge.expectedCurrentGate, 'private_observation_incomplete')
...
test('renders Markdown and verifies CLI packet without private values', async () => {
  ...
  try {
    ...
    assert.match(markdown, /Baseline packet verification required: true/)
    assert.match(markdown, /Ready to generate and verify owner-safe baseline packet: false/)
    ...
    assert.match(markdown, /--verify "<owner-safe-baseline-packet\.json>"/)
