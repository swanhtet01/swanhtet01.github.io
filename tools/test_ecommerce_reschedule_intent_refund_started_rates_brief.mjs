...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceRescheduleIntentRefundStartedRatesBrief } from './ecommerce-reschedule-intent-refund-started-rates-brief.ts'`,
...
const { projectEcommerceRescheduleIntentRefundStartedRatesBrief } = await import(
...
let intentId = 0
function rescheduleIntent({ refundStarted = false } = {}) {
  intentId++
  return {
...
{
  const r = projectEcommerceRescheduleIntentRefundStartedRatesBrief(state())
...
{
  const r = projectEcommerceRescheduleIntentRefundStartedRatesBrief(state([
    rescheduleIntent({ refundStarted: true }),
  ]))
...
{
  const r = projectEcommerceRescheduleIntentRefundStartedRatesBrief(state([rescheduleIntent()]))
...
console.log(`ecommerce-reschedule-intent-refund-started-rates-brief: ${checks} checks passed`)
