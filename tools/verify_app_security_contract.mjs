...
requireContract('managed schema contract advances through additive v2 through v6 migrations', /TRIAL_SCHEMA_VERSION = 6/.test(trialStore) && /set schema_version = 2/.test(decisionMigration) && /schema_version = 1/.test(decisionMigration) && /set schema_version = 3/.test(websiteMigration) && /schema_version = 2/.test(websiteMigration) && /set schema_version = 4/.test(hardeningMigration) && /schema version 3/.test(hardeningMigration) && /set schema_version = 5/.test(readCapabilityMigration) && /schema version 4/.test(readCapabilityMigration) && /set schema_version = 6/.test(activationMigration) && /schema version 5/.test(activationMigration) && /workspace_state\.surface \|\| '\.read'/.test(readCapabilityMigration) && /'approvals\.read'/.test(readCapabilityMigration))
requireContract('managed runtime requires a database-enforced active workspace access gate', /workspace_access_controls/.test(activationMigration) && /workspace access can only transition from active to suspended/.test(activationMigration) && /create function app_private\.workspace_is_active/.test(activationMigration) && (activationMigration.match(/as restrictive/g) || []).length === 4 && /workspace_memberships_access_gate/.test(activationMigration) && /workspace_state_access_gate/.test(activationMigration) && /workspace_events_access_gate/.test(activationMigration) && /approval_requests_access_gate/.test(activationMigration) && /join app_private\.workspace_access_controls access_control/.test(trialStore) && /access_control\.status = 'active'/.test(trialStore))
...
requireContract('managed database readiness validator targets exact PostgreSQL and schema contracts', /CONTRACT = "supermega_private_trial_database_v6"/.test(databaseValidator) && /EXPECTED_POSTGRES_MAJOR = 17/.test(databaseValidator) && /pg_db_role_setting/.test(databaseValidator) && /SCHEMA_VERSION = 6/.test(databaseValidator) && /complete v6 schema contract/.test(databaseValidator) && /workspace_access_controls/.test(databaseValidator) && /workspace_is_active/.test(databaseValidator) && /RESTRICTIVE/.test(databaseValidator) && /EXPECTED_POLICY_FINGERPRINTS/.test(databaseValidator) && /security_constraints_exact/.test(databaseValidator))
...
requireContract('managed AI context is owner-consented, summary-only, tenant-bound, and revision-bound',
  ...
  && /I approve these summarized source counts/.test(managedContextConsent)
  && /Keep managed context/.test(managedContextConsent))
