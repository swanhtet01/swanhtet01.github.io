...
requireContract('managed schema contract advances through additive v2 through v8 migrations', /TRIAL_SCHEMA_VERSION = 8/.test(trialStore) && /set schema_version = 2/.test(decisionMigration) && /schema_version = 1/.test(decisionMigration) && /set schema_version = 3/.test(websiteMigration) && /schema_version = 2/.test(websiteMigration) && /set schema_version = 4/.test(hardeningMigration) && /schema version 3/.test(hardeningMigration) && /set schema_version = 5/.test(readCapabilityMigration) && /schema version 4/.test(readCapabilityMigration) && /set schema_version = 6/.test(activationMigration) && /schema version 5/.test(activationMigration) && /set schema_version = 7/.test(workspaceDiscoveryMigration) && /schema version 6/.test(workspaceDiscoveryMigration) && /set schema_version = 8/.test(rlsInitplanMigration) && /schema version 7/.test(rlsInitplanMigration) && (rlsInitplanMigration.match(/drop policy/g) || []).length === 10 && !rlsInitplanMigration.replace(/\(\s*select\s+current_setting\(/g, '').includes('current_setting(') && /workspace_state\.surface \|\| '\.read'/.test(readCapabilityMigration) && /'approvals\.read'/.test(readCapabilityMigration))
...
requireContract('managed database readiness validator targets exact PostgreSQL and schema contracts', /CONTRACT = "supermega_private_trial_database_v8"/.test(databaseValidator) && /EXPECTED_POSTGRES_MAJOR = 17/.test(databaseValidator) && /pg_db_role_setting/.test(databaseValidator) && /SCHEMA_VERSION = 8/.test(databaseValidator) && /complete v8 schema contract/.test(databaseValidator) && /select current_setting\('app\.workspace_id'/.test(databaseValidator) && /workspace_access_controls/.test(databaseValidator) && /workspace_is_active/.test(databaseValidator) && /RESTRICTIVE/.test(databaseValidator) && /EXPECTED_POLICY_FINGERPRINTS/.test(databaseValidator) && /security_constraints_exact/.test(databaseValidator))
