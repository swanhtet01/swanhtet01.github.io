...
const [runtime, supabaseAuth, cloudRuntime, agentGovernance, vercelEntry, portableEntry, trialRuntime, trialStore, commerceRuntime, productionRuntime, websiteRuntime, managedTrialClient, coreApp, websiteWorkspaceHook, rolePreflight, foundationMigration, decisionMigration, websiteMigration, hardeningMigration, readCapabilityMigration, databaseValidator, databaseActivator, liveVerifier, workflow, requirements, dockerfile, appEnvironment] = await Promise.all([
...
requireContract('hosted agent scheduler has bounded jobs and explicit egress', /SUPERMEGA_CLOUD_TASKS_ALLOWED_HOSTS/.test(cloudRuntime) && /AGENT_AUTOMATION_LANES/.test(cloudRuntime) && /"queue": \("task_triage", "ops_watch"\)/.test(agentGovernance) && /"daily": \("founder_brief", "github_release_watch"\)/.test(agentGovernance) && /ProxyHandler\(\{\}\)/.test(cloudRuntime) && /_NoRedirectHandler/.test(cloudRuntime))
