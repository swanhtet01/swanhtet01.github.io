...
requireContract('managed schema contract advances through additive v2 through v7 migrations', /TRIAL_SCHEMA_VERSION = 7/.test(trialStore) && /set schema_version = 2/.test(decisionMigration) && /schema_version = 1/.test(decisionMigration) && /set schema_version = 3/.test(websiteMigration) && /schema_version = 2/.test(websiteMigration) && /set schema_version = 4/.test(hardeningMigration) && /schema version 3/.test(hardeningMigration) && /set schema_version = 5/.test(readCapabilityMigration) && /schema version 4/.test(readCapabilityMigration) && /set schema_version = 6/.test(activationMigration) && /schema version 5/.test(activationMigration) && /set schema_version = 7/.test(workspaceDiscoveryMigration) && /schema version 6/.test(workspaceDiscoveryMigration) && /workspace_state\.surface \|\| '\.read'/.test(readCapabilityMigration) && /'approvals\.read'/.test(readCapabilityMigration))
