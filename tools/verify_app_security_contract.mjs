...
const websiteBriefProvider = await read('supermega_runtime/website_brief_provider.py')
const websiteAiBriefClient = await read('showroom/src/products/website/website-ai-brief.ts')
const websiteStarterSetup = await read('showroom/src/products/website/WebsiteStarterSetup.tsx')
const plantJobRequestProvider = await read('supermega_runtime/plant_job_request_provider.py')
const plantAiJobRequestClient = await read('showroom/src/core/plant-ai-job-request.ts')
const channelOrderContract = await read('showroom/src/core/channel-order-intake.ts')
...
const shopServiceSchedule = await read('showroom/src/core/shop-service-scheduling.ts')
const shopServiceScheduleUi = await read('showroom/src/core/ShopServiceSchedule.tsx')
...
const localOrderIntakeRoute = runtime.slice(
  ...
  runtime.indexOf('@app.post("/api/local/v1/website/brief-drafts")'),
)
const localWebsiteBriefRoute = runtime.slice(
  runtime.indexOf('@app.post("/api/local/v1/website/brief-drafts")'),
  runtime.indexOf('@app.post("/api/local/v1/plant/job-request-drafts")'),
)
const localPlantJobRequestRoute = runtime.slice(
  runtime.indexOf('@app.post("/api/local/v1/plant/job-request-drafts")'),
...
requireContract('local AI order intake is fixed-loopback, Llama-only, scale-to-zero, and never falls through to a paid key',
  orderIntakeProvider.includes('OLLAMA_CHAT_URL = "http://127.0.0.1:11434/api/chat"')
  && orderIntakeProvider.includes('LOCAL_ORDER_INTAKE_MODELS = frozenset({"llama3.2:1b", "llama3.2:3b"})')
  && orderIntakeProvider.includes('if _hosted_runtime() or str(os.getenv("SUPERMEGA_OLLAMA_ENABLED")')
  && orderIntakeProvider.includes('"think": False')
  && orderIntakeProvider.includes('"keep_alive": 0')
  && orderIntakeProvider.includes('OLLAMA_ORDER_INTAKE_TIMEOUT_SECONDS = 60.0')
  ...
  && orderIntakeProvider.includes('except OrderIntakeProviderError:\n            return None')
  && !orderIntakeProvider.includes('SUPERMEGA_OLLAMA_URL')
  && orderIntakeContract.includes('OrderIntakeModelProvider = Literal["openai", "ollama-local"]')
...
requireContract('local Website AI is grounded, fixed-loopback, scale-to-zero, and review-only',
  websiteBriefProvider.includes('OLLAMA_CHAT_URL = "http://127.0.0.1:11434/api/chat"')
  && websiteBriefProvider.includes('LOCAL_WEBSITE_BRIEF_MODELS = frozenset({"llama3.2:1b", "llama3.2:3b"})')
  && websiteBriefProvider.includes('if _hosted_runtime() or str(os.getenv("SUPERMEGA_OLLAMA_ENABLED")')
  && websiteBriefProvider.includes('"think": False')
  && websiteBriefProvider.includes('"keep_alive": 0')
  && websiteBriefProvider.includes('"format": schema')
  && websiteBriefProvider.includes('self._lock = asyncio.Lock()')
  && websiteBriefProvider.includes('async with self._lock:')
  && websiteBriefProvider.includes('_quote_exists(request.source_text, record.quote, record.occurrence)')
  && websiteBriefProvider.includes('_normalized_line(record.quote) == normalized')
  && websiteBriefProvider.includes('proof must be a supportable fact explicitly stated by the owner')
  && websiteBriefProvider.includes('return OllamaWebsiteBriefProvider.from_environment()')
  && !websiteBriefProvider.includes('OPENAI_API_KEY')
  && !websiteBriefProvider.includes('SUPERMEGA_OLLAMA_URL')
  && !websiteBriefProvider.includes('"tools"')
  && runtime.includes('configured_website_brief_provider()')
  && runtime.includes('not database_url')
  && runtime.includes('not _flag("SUPERMEGA_TRIAL_WRITES_ENABLED")')
  && runtime.includes('getattr(website_brief_provider, "provider_id", "") == "ollama-local"')
  && localWebsiteBriefRoute.includes('_local_website_brief_request_allowed(request, origins)')
  && localWebsiteBriefRoute.includes('set(body) != {"source_label", "brief"}')
  && localWebsiteBriefRoute.includes('WebsiteBriefRequest.model_validate')
  && localWebsiteBriefRoute.includes('workspace_id="local-demo-website"')
  && localWebsiteBriefRoute.includes('actor_id="local-human-review"')
  && localWebsiteBriefRoute.includes('"raw_brief_retained": False')
  && localWebsiteBriefRoute.includes('"website_changes_performed": 0')
  && localWebsiteBriefRoute.includes('"publish_performed": False')
  && localWebsiteBriefRoute.includes('"external_writes_performed": False')
  && !/store\.(?:apply_command|create_approval|decide_approval)/.test(localWebsiteBriefRoute)
  && websiteAiBriefClient.includes("'/api/local/v1/website/brief-drafts'")
  && websiteAiBriefClient.includes("'x-supermega-local-review': 'website-brief-v1'")
  && websiteAiBriefClient.includes("credentials: 'omit'")
  && websiteAiBriefClient.includes('body.raw_brief_retained !== false')
  && websiteAiBriefClient.includes('body.website_changes_performed !== 0')
  && websiteAiBriefClient.includes('body.publish_performed !== false')
  && websiteAiBriefClient.includes('body.external_writes_performed !== false')
  && websiteStarterSetup.includes("await import('./website-ai-brief')")
  && websiteStarterSetup.includes("setSourceText('')")
  && websiteStarterSetup.includes('Check every detail, then make the preview.')
  && !websiteAiBriefClient.includes('localStorage')
  && !websiteAiBriefClient.includes('sessionStorage'))
requireContract('local Plant AI is source-bound, fixed-loopback, scale-to-zero, and review-only',
  plantJobRequestProvider.includes('OLLAMA_CHAT_URL = "http://127.0.0.1:11434/api/chat"')
  && plantJobRequestProvider.includes('LOCAL_PLANT_JOB_MODELS = frozenset({"llama3.2:1b", "llama3.2:3b"})')
  && plantJobRequestProvider.includes('if _hosted_runtime() or str(os.getenv("SUPERMEGA_OLLAMA_ENABLED")')
  && plantJobRequestProvider.includes('!= "local-only"')
  && plantJobRequestProvider.includes('value not in source_text')
  && plantJobRequestProvider.includes('Never infer a date from words such as tomorrow or Friday.')
  && plantJobRequestProvider.includes('"think": False')
  && plantJobRequestProvider.includes('"keep_alive": 0')
  && plantJobRequestProvider.includes('"format": PlantJobRequestModelExtraction.model_json_schema')
  && plantJobRequestProvider.includes('self._lock = asyncio.Lock()')
  && plantJobRequestProvider.includes('async with self._lock:')
  && !plantJobRequestProvider.includes('OPENAI_API_KEY')
  && !plantJobRequestProvider.includes('ANTHROPIC_API_KEY')
  && !plantJobRequestProvider.includes('SUPERMEGA_OLLAMA_URL')
  && !plantJobRequestProvider.includes('"tools"')
  && runtime.includes('configured_plant_job_request_provider()')
  && runtime.includes('getattr(plant_job_request_provider, "provider_id", "") == "ollama-local"')
  && localPlantJobRequestRoute.includes('_local_plant_job_request_allowed(request, origins)')
  && localPlantJobRequestRoute.includes('set(body) != {"source_label", "request_text"}')
  && localPlantJobRequestRoute.includes('PlantJobRequest.model_validate')
  && localPlantJobRequestRoute.includes('workspace_id="local-demo-plant"')
  && localPlantJobRequestRoute.includes('actor_id="local-human-review"')
  && localPlantJobRequestRoute.includes('"raw_request_retained": False')
  && localPlantJobRequestRoute.includes('"jobs_created": 0')
  && localPlantJobRequestRoute.includes('"schedule_changes_performed": 0')
  && localPlantJobRequestRoute.includes('"material_actions_performed": 0')
  && localPlantJobRequestRoute.includes('"equipment_actions_performed": 0')
  && localPlantJobRequestRoute.includes('"external_writes_performed": False')
  && !/store\.(?:apply_command|create_approval|decide_approval)/.test(localPlantJobRequestRoute)
  && plantAiJobRequestClient.includes("'/api/local/v1/plant/job-request-drafts'")
  && plantAiJobRequestClient.includes("'x-supermega-local-review': 'plant-job-request-v1'")
  && plantAiJobRequestClient.includes("credentials: 'omit'")
  && plantAiJobRequestClient.includes('body.raw_request_retained !== false')
  && plantAiJobRequestClient.includes('body.jobs_created !== 0')
  && plantAiJobRequestClient.includes('body.schedule_changes_performed !== 0')
  && plantAiJobRequestClient.includes('body.material_actions_performed !== 0')
  && plantAiJobRequestClient.includes('body.equipment_actions_performed !== 0')
  && plantAiJobRequestClient.includes('body.external_writes_performed !== false')
  && coreApp.includes("await import('./plant-ai-job-request')")
  && coreApp.includes('Source-checked draft ready. Review before creating anything.')
  && coreApp.includes("evidenceReferenceSuggestion: demandSignal?.evidenceReference ?? (id.startsWith('JOB-AI-')")
  && !plantAiJobRequestClient.includes('localStorage')
  && !plantAiJobRequestClient.includes('sessionStorage'))
requireContract('reviewed message fulfilment is source-bound through Shop confirmation',
  channelOrderContract.includes("supermega.channel_order_draft.v2")
  && channelOrderContract.includes("channelOrderFields = ['customer', 'sku', 'quantity', 'payment', 'fulfilment']")
  && channelOrderContract.includes('fulfilment: draft.fulfilment,')
  && channelOrderContract.includes('channelOrderFulfilments.includes(draft.fulfilment')
  && channelOrderIntakeUi.includes("setFulfilment(draft.fulfilment ?? '')")
  && coreApp.includes('setFulfilment(draft.fulfilment)')
  && coreApp.includes('fulfilment !== sourceDraft.fulfilment')
  && coreApp.includes('Fulfilment changed. The message source link was removed; review this as a manual order.'))
...
requireContract('Spa staff review handoff preserves the complete expiring contract in one explicit local-only private download',
  /expiresAt - requestedAt !== 15 \* 60 \* 1000/.test(managedTrialClient)
  && /candidate\.capabilities\.join\(','\) !== expectedCapabilities/.test(managedTrialClient)
  && /required_checks: string\[\]/.test(managedTrialClient)
  && /forbidden_until_confirmed: string\[\]/.test(managedTrialClient)
  && /!activation\.required_checks\.length/.test(managedTrialClient)
  && /!activation\.forbidden_until_confirmed\.length/.test(managedTrialClient)
  && /_REQUIRED_ACTIVATION_CHECKS/.test(spaStaffAccess)
  && /_REVIEW_FORBIDDEN_ACTIONS/.test(spaStaffAccess)
  && /function downloadSpaStaffReview\(\)/.test(productOnboardingPage)
  && /new Blob\(/.test(productOnboardingPage)
  && /URL\.createObjectURL\(blob\)/.test(productOnboardingPage)
  && /URL\.revokeObjectURL\(href\)/.test(productOnboardingPage)
  && /supermega-spa-staff-review-/.test(productOnboardingPage)
  && /Download private setup file/.test(productOnboardingPage)
  && /Contains this staff name and work email\./.test(productOnboardingPage)
  && !/navigator\.sendBeacon|XMLHttpRequest/.test(productOnboardingPage))
...
requireContract('Spa staff invitation handoff is privacy-minimal, exact-redirect, preflight-gated, and no-send',
  /SPA_STAFF_INVITATION_HANDOFF_CONTRACT = "supermega\.managed_spa_staff_invitation_handoff\.v1"/.test(spaStaffAccess)
  && /SPA_STAFF_INVITATION_REDIRECT_URL = "https:\/\/app\.supermega\.dev\/account\/setup\?mode=invite"/.test(spaStaffAccess)
  && /"status": "operator_preflight_required"/.test(spaStaffAccess)
  && /"displayNameDigest": _digest\(review\["candidate"\]\["display_name"\]\)/.test(spaStaffAccess)
  && /email_digest = spa_staff_email_digest\(review\["candidate"\]\["email"\]\)/.test(spaStaffAccess)
  && /SPA_STAFF_INVITATION_HANDOFF_CONTRACT}:\{review\['workspace_id'\]}:\{email_digest\}/.test(spaStaffAccess)
  && /"emailDigest": email_digest/.test(spaStaffAccess)
  && /"exact_redirect_allowlist_evidence"/.test(spaStaffAccess)
  && /"custom_smtp_delivery_evidence"/.test(spaStaffAccess)
  && /"owner_provider_call_confirmation"/.test(spaStaffAccess)
  && /"automatic_provider_retry"/.test(spaStaffAccess)
  && /"providerRequestAuthorized": False/.test(spaStaffAccess)
  && /"externalProviderRequestsPerformed": False/.test(spaStaffAccess)
  && !/from urllib|import urllib|urlopen\(|build_opener\(/.test(spaStaffAccess))
requireContract('Spa staff invitation receipt is provider-evidence only and never grants membership',
  /SPA_STAFF_INVITATION_RECEIPT_CONTRACT = "supermega\.managed_spa_staff_invitation_receipt\.v1"/.test(spaStaffAccess)
  && /def record_spa_staff_invitation_result\(/.test(spaStaffAccess)
  && /"status": "invited_pending_first_sign_in"/.test(spaStaffAccess)
  && /"verification": "requery_before_membership"/.test(spaStaffAccess)
  && /"fresh_staff_sign_in"/.test(spaStaffAccess)
  && /"emailDeliveryVerified": False/.test(spaStaffAccess)
  && /"inviteAcceptedByUser": False/.test(spaStaffAccess)
  && /"membershipWritten": False/.test(spaStaffAccess)
  && /"externalProviderRequestsPerformed": True/.test(spaStaffAccess)
  && /Invited Supabase email does not match the reviewed staff email\./.test(spaStaffAccess))
requireContract('Spa staff invitation requires durable fresh-owner authorization before any consumable provider attempt',
  /SPA_STAFF_INVITATION_AUTHORIZATION_CONTRACT = "supermega\.managed_spa_staff_invitation_authorization\.v1"/.test(spaStaffAccess)
  && /def spa_staff_invitation_approval_id\(/.test(spaStaffAccess)
  && /def authorize_spa_staff_invitation\(/.test(managedActivation)
  && /def inspect_spa_staff_invitation\(/.test(managedActivation)
  && /supabase_session_is_active/.test(managedActivation)
  && /_owner_controls_staff_access\(rows, handoff\)/.test(managedActivation)
  && /"authorizedProviderRequestCount": 1/.test(managedActivation)
  && /"authorizationConsumptionRequired": True/.test(managedActivation)
  && /"automaticRetryAllowed": False/.test(managedActivation)
  && /"providerRequestPerformed": False/.test(managedActivation)
  && /"externalProviderRequestsPerformed": False/.test(managedActivation)
  && !/inviteUserByEmail/.test(managedActivation))
requireContract('Spa staff invitation authorization is consumed once before provider transport and replays fail closed',
  /SPA_STAFF_INVITATION_ATTEMPT_CONTRACT = "supermega\.managed_spa_staff_invitation_attempt\.v1"/.test(spaStaffAccess)
  && /SPA_STAFF_INVITATION_ATTEMPT_RESULT_CONTRACT = "supermega\.managed_spa_staff_invitation_attempt_result\.v1"/.test(spaStaffAccess)
  && /def spa_staff_invitation_attempt_id\(/.test(spaStaffAccess)
  && /def spa_staff_invitation_attempt_result_id\(/.test(spaStaffAccess)
  && /def claim_spa_staff_invitation_attempt\(/.test(managedActivation)
  && /def record_spa_staff_invitation_attempt_result\(/.test(managedActivation)
  && /_staff_invitation_claim_matches/.test(managedActivation)
  && /_staff_invitation_result_matches/.test(managedActivation)
  && /company\.spa_staff_invitation\.attempt_claimed/.test(managedActivation)
  && /company\.spa_staff_invitation\.result_recorded/.test(managedActivation)
  && /"providerRequestMayStart": not replayed and not result_recorded/.test(managedActivation)
  && /"reconciliationRequired": replayed and not result_recorded/.test(managedActivation)
  && /"unknown_after_claim"/.test(managedActivation)
  && /"performed_succeeded"/.test(managedActivation)
  && /"automaticRetryAllowed": False/.test(managedActivation)
  && /"externalProviderRequestsPerformedByRecorder": False/.test(managedActivation)
  && /validate_spa_staff_invitation_receipt/.test(managedActivation)
  && /Staff invitation authorization was not consumed before provider evidence\./.test(managedActivation)
  && !/inviteUserByEmail/.test(managedActivation))
...
requireContract('public browser quarantine is compatibility, idempotence, drift, and server-path verified',
  ...
  && rootPackage.scripts?.['app:verify']?.includes('database:public-browser-quarantine:verify'))
...
requireContract('Spa therapist and viewer reads are field-restricted and treatment completion is narrow', trialRuntime.includes('_spa_schedule_privacy_scope') && trialRuntime.includes('_project_spa_schedule_for_read') && trialRuntime.includes('projected.pop("clients", None)') && trialRuntime.includes('key not in {"contact", "appointmentUpdates"}') && trialRuntime.includes('safe_state.pop("serviceSchedule", None)') && trialRuntime.includes('/commerce/service-schedule/actions/complete-treatment') && trialRuntime.includes('_restricted_spa_role(readiness) != "therapist"') && trialRuntime.includes('"privacy_scope": "restricted"') && managedTrialClient.includes('readRestrictedShopServiceSchedule') && managedTrialClient.includes("request.identity.access === 'spa-therapist'"))
requireContract('Spa client retention export and anonymization are owner-gated and server-verified',
  trialRuntime.includes('_SPA_OWNER_SCHEDULE_EVENTS')
  && trialRuntime.includes('_spa_owner_schedule_event')
  && trialRuntime.includes('_require_spa_owner_schedule_action')
  && trialRuntime.includes('spa_owner_action_required')
  && trialRuntime.includes('spa_privacy_action_route_required')
  && trialRuntime.includes('latest_event["happenedAt"] = _current_utc_timestamp()')
  && trialRuntime.includes('{"company.control.approve", "company.write"}')
  && trialRuntime.includes('"updatedBy": principal.actor_id')
  && trialRuntime.includes('"anonymizedBy": principal.actor_id')
  && commerceRuntime.includes('_service_client_export_csv')
  && commerceRuntime.includes('sha256(_service_client_export_csv(before).encode("utf-8")).hexdigest()')
  && commerceRuntime.includes('financial_records_closed')
  && commerceRuntime.includes('retention_elapsed')
  && commerceRuntime.includes('evidence_identity_free')
  && commerceRuntime.includes('_service_events_contain_identifier')
  && commerceRuntime.includes('initialize the reviewed service schedule before saving operating evidence')
  && commerceRuntime.includes('service schedule initialization cannot pre-approve a client retention policy')
  && commerceRuntime.includes('expected_reason')
  && commerceRuntime.includes('"client_anonymized"')
  && shopServiceSchedule.includes('shopServiceClientAnonymizationReadiness')
  && shopServiceSchedule.includes('anonymizeShopServiceClient')
  && shopServiceSchedule.includes('evidenceContainsIdentifier')
  && shopServiceSchedule.includes('existingClient?.anonymizedAt')
  && shopServiceSchedule.includes("/^[=+\\-@\\t\\r]/.test(text)")
  && shopServiceScheduleUi.includes("if (!await commit(next, 'Client export receipt recorded.')) return")
  && shopServiceScheduleUi.indexOf("if (!await commit(next, 'Client export receipt recorded.')) return") < shopServiceScheduleUi.indexOf('URL.createObjectURL(new Blob([csv]')
  && shopServiceScheduleUi.includes('Review permanent anonymization')
  && shopServiceScheduleUi.includes('schedule.clients.filter((client) => !client.anonymizedAt)')
  && shopServiceScheduleUi.includes('Confirm anonymization'))
