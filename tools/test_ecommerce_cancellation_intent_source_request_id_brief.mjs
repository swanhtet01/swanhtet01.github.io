...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceCancellationIntentSourceRequestIdBrief } from './ecommerce-cancellation-intent-source-request-id-brief.ts'`,
...
const { projectEcommerceCancellationIntentSourceRequestIdBrief } = await import(
...
let intentId = 0
function cancellationIntent(sourceRequestId = 'REQ-DEFAULT') {
  intentId++
  return {
...
// 1. Empty state
{
  const r = projectEcommerceCancellationIntentSourceRequestIdBrief(state())
...
{
  const r = projectEcommerceCancellationIntentSourceRequestIdBrief(state([cancellationIntent('REQ-001')]))
...
{
  const r = projectEcommerceCancellationIntentSourceRequestIdBrief(state([
    cancellationIntent('REQ-003'),
    cancellationIntent('REQ-004'),
  ]))
...
{
  const r = projectEcommerceCancellationIntentSourceRequestIdBrief(state([
    cancellationIntent('REQ-005'),
    cancellationIntent('REQ-006'),
    cancellationIntent('REQ-005'),
  ]))
...
{
  const r = projectEcommerceCancellationIntentSourceRequestIdBrief(state([
    cancellationIntent('REQ-007'),
    cancellationIntent('REQ-008'),
    cancellationIntent('REQ-009'),
  ]))
...
console.log(`ecommerce-cancellation-intent-source-request-id-brief: ${checks} checks passed`)
