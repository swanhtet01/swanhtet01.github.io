...
const bundle = await build({
  stdin: {
    contents: `export { projectEcommerceAmendmentIntentFromFulfilmentRatesBrief } from './ecommerce-amendment-intent-from-fulfilment-rates-brief.ts'`,
...
const { projectEcommerceAmendmentIntentFromFulfilmentRatesBrief } = await import(
...
{
  const r = projectEcommerceAmendmentIntentFromFulfilmentRatesBrief(state())
...
// 2. Single from-delivery — 3 checks
{
  const r = projectEcommerceAmendmentIntentFromFulfilmentRatesBrief(state([
    amendmentIntent({ fromFulfilment: 'delivery' }),
  ]))
...
// 3. Single from-pickup — 3 checks
{
  const r = projectEcommerceAmendmentIntentFromFulfilmentRatesBrief(state([amendmentIntent()]))
  check(r.totalIntents === 1, 'pickup:total')
  check(r.pickupCount === 1, 'pickup:count')
  check(r.pickupRate === 1, 'pickup:rate')
}

// 4. 2 from-delivery — 3 checks
{
  const r = projectEcommerceAmendmentIntentFromFulfilmentRatesBrief(state([
    amendmentIntent({ fromFulfilment: 'delivery' }),
    amendmentIntent({ fromFulfilment: 'delivery' }),
...
console.log(`ecommerce-amendment-intent-from-fulfilment-rates-brief: ${checks} checks passed`)
