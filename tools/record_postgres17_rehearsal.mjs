...
export async function implementationEvidence(repositoryRoot = root) {
  const runtimeFiles = await recursiveFiles(resolve(repositoryRoot, 'supermega_runtime'))
  const paths = [
    ...implementationRoots.map((path) => resolve(repositoryRoot, path)),
    ...runtimeFiles,
  ].map((path) => relative(repositoryRoot, path).split(sep).join('/')).sort()
  const records = []
  for (const path of paths) records.push([path, sha256(await readFile(resolve(repositoryRoot, path)))])
  return {
    digest: `sha256:${sha256(records.map(([path, digest]) => `${path}\0${digest}`).join('\n'))}`,
...
export function buildSanitizedProof(raw, context) {
  ...
  if (raw.migrations?.count !== migrations.length || raw.migrations?.schema_version !== 5 || raw.migrations?.production_validator_ready !== true) fail('database_rehearsal_migrations_invalid')
  ...
  if (raw.recovery?.backup_nonempty !== true || raw.recovery?.restored_schema_version !== 5) fail('database_rehearsal_recovery_invalid')
...
export function validateSanitizedProof(proof, currentImplementation) {
  ...
  if (proof.implementationDigest !== currentImplementation.digest || proof.implementationFileCount !== currentImplementation.fileCount) fail('database_rehearsal_evidence_stale')
  ...
  if (proof.migration?.count !== migrations.length || proof.migration?.schemaVersion !== 5 || proof.migration?.productionValidatorReady !== true) fail('database_rehearsal_evidence_migrations_invalid')
